class RestScene extends Phaser.Scene {
    ...
    create() {
        ...
        // Rest option
        const restBtn = this.add.text(W / 2, 320, `❤️ 휴식 — HP ${healAmount} 회복`, {
            fontSize: '20px', fontFamily: 'monospace', color: '#44ff88',
            backgroundColor: '#1a3322', padding: { x: 24, y: 12 }
        }).setOrigin(0.5).setInteractive({ useHandCursor: true });

        restBtn.on('pointerover', () => restBtn.setColor('#ffffff'));
        restBtn.on('pointerout', () => restBtn.setColor('#44ff88'));
        ...
        if (this.playerState.gold >= 15) {
            const upgradeBtn = this.add.text(W / 2, 400, `💖 단련 — 최대 HP +5 (15G)`, {
                fontSize: '18px', fontFamily: 'monospace', color: '#ff88cc',
                backgroundColor: '#2a1a2a', padding: { x: 20, y: 10 }
            }).setOrigin(0.5).setInteractive({ useHandCursor: true });

            upgradeBtn.on('pointerover', () => upgradeBtn.setColor('#ffffff'));
            upgradeBtn.on('pointerout', () => upgradeBtn.setColor('#ff88cc'));
            upgradeBtn.on('pointerdown', () => {
                this.playerState.gold -= 15;
                this.playerState.maxHp += 5;
                this.playerState.hp += 5;
                upgradeBtn.disableInteractive();
                upgradeBtn.setText('💖 단련 완료!');
                this.time.delayedCall(500, () => this._goBack());
            });
        }
