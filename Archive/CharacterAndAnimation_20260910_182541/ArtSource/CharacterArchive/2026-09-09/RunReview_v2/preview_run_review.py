...
OLD=ROOT/'ArtSource/CharacterArchive/2026-09-09/RunReview_v1/CompactSurvivor_RunReview.blend'
CACHE=ROOT/'Library/CodexBlender/RunReview_v2';CACHE.mkdir(exist_ok=True)
bpy.ops.wm.open_mainfile(filepath=str(OUT/'CompactSurvivor_RunReview.blend'))
...
if '--poses' in sys.argv:
    ...
    scene.cycles.samples=16;scene.render.image_settings.file_format='PNG';scene.render.filepath=str(OUT/'RunPoses_v2.png')
    ...
    movie_settings()
    for title,action,speed in [('Previous',old,.35/(.4*24/30)),('Revised',new,stats['nominal_forward_speed_mps'])]:
        use(action)
        ...
        if title=='Revised':bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'RunPreview_v2.blend'))
    ...
    scene.frame_start=1;scene.frame_end=240;path=OUT/'RunComparison_v2.mp4';scene.render.filepath=str(path)
    ...
    (OUT/'BodyPreviewCheck.json').write_text(json.dumps({'frames':240,'fps':30,'seconds':8.0,
      'left':'Run v1','right':'Run v2','same_camera':True,'each_clip_natural_cadence':True,
