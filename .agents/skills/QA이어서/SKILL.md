...
demo13-flashlight의 **"Codex가 명령을 내려 게임을 실제로 플레이시키고, 밸런스·막힘 데이터를 뽑아, 자동으로 고치는"** 시스템.
...
- **게임 빌드에 API 키를 넣지 않는다.** Codex 연결은 **파일 채널**(qa-command/status/result/blocked/resume).
...
| `.Codex/agents/qa-runner.md` | QA 실행·판정 전용(코드 안 고침) |
| `.Codex/agents/dev-fixer.md` | 수정 전용(QA 안 돌림) |
| `.Codex/skills/qa/` · `qa-loop/` | 지휘 / 자동 루프 |
...
- 커밋·푸시는 **사용자 요청 시만**. 메시지 끝 `Co-Authored-By: Codex Opus 5 <noreply@anthropic.com>`.
