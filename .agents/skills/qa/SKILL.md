...
| 파일 | 방향 | 내용 |
|------|------|------|
| `qa-command.json` | **Codex → 게임** | 실행할 시퀀스 |
| `qa-status.json` | 게임 → Codex | 진행 상황 하트비트(1초 갱신) |
| `qa-result-latest.json` | 게임 → Codex | 런 종료 결과(지표·이상 전부) |
| `qa-blocked.json` | 게임 → Codex | **막힘** — 봇이 응답을 기다리는 중 |
| `qa-resume.json` | **Codex → 게임** | 막힘에 대한 지시 |
