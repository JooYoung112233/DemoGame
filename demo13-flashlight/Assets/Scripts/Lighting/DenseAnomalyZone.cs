...
/// 트리거 3D Collider 필요(Box 등). 겹치는 구간이 여러 개면 가장 큰 intensity 적용.
/// DenseAnomalyController가 매 프레임 들어간 구간들의 max를 목표 강도로 반영(부드럽게 lerp).
/// (2026-09-12 2D 정리 — Collider2D·OnTrigger*2D에서 3D로 이식)
/// </summary>
...
public class DenseAnomalyZone : MonoBehaviour
{
    ...
    static void CreateZone()
    {
        ...
        if (sv != null) { var p = sv.pivot; p.y = 0f; go.transform.position = p; }
