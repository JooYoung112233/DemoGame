...
COUNT=24; DUTY=.40; STRIDE=.35; LIFT=.16; SPEED=STRIDE/(DUTY*COUNT/30)
pivots={}
...
def foot_goal(phase,s):
    ...
    if stance:
        ...
        pitch=-8*(1-smooth(0,.18,q))+32*smooth(.42,1,q)
    else:
        u=(phi-DUTY)/(1-DUTY); tangent=STRIDE/DUTY*(1-DUTY)
        base=.003+hermite(STRIDE/2,-STRIDE/2,tangent,tangent,u)
        # Early heel recovery, then extend the lower leg before the next contact.
        lift=LIFT*math.sin(math.pi*u)**1.2
        pitch=32+16*math.sin(math.pi*u)-40*smooth(0,1,u)
...
def hip_xy_rot(phase):
    t=math.tau*phase
    return V((.012*math.sin(t),-.026,0)),world_rot(x=3,y=1.8*math.sin(t),z=-7*math.cos(t))
corrections=[]
def pose(phase):
    ...
    hip.z=.512-.025*math.cos(2*t-1.5)
    ceiling=hip.z
    for s in ('L','R'):
    ...
    follow('Chest',d,world_rot(x=3+.9*math.sin(2*t-.45),y=.6*math.sin(t-.25),z=10*math.cos(t-.18)))
    follow('Neck',d)
    ...
    for s,sign in [('L',1),('R',-1)]:
        ...
        follow('UpperArm.'+s,d,world_rot(x=34*swing,y=sign*-2))
        follow('Forearm.'+s,d,world_rot(x=-58+16*swing))
...
report={'frames':[1,COUNT+1],'fps':30,'cycle_seconds':COUNT/30,'nominal_forward_speed_mps':SPEED,
