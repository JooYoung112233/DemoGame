...
| 2026-07-11 | **갭 분석 통합 순서 구현 진행: ⑨ 완료.** ✅ ⑨아이템 도감 — **독립 패널 `N`키**(TraitPanelUI(K) 자가부트 패턴, sortingOrder 920, ESC 닫기), **좌 카테고리 탭(ItemCategory 7종, 발견/전체 표기) + 가운데 아이콘 격자 + 우 상세**, **미발견 = 검은 실루엣 + 이름 `???`**(설명도 가림, 클릭해도 상세 없음 — 수집 동기 최대). **발견 훅 = `InventoryGrid.OnItemPlaced` 콜백**(옵트인·기본 null)에 **플레이어 3격자만 구독**(`PlayerInventory.Awake`) → 줍기·루팅 드래그·자동배치·구매·제작 등 모든 획득 경로를 한 지점에서 포착. 신규 `Inventory/CodexManager.cs`(발견 set·토스트·영속·새게임 리셋) + `UI/CodexUI.cs`. `SaveManager.discoveredItems` 영속 + `ResetToNewGame` 리셋, `UIManager` CloseTopmost/IsAnyUIOpen 등록. F1 "전체 발견 처리"/"도감 초기화". 1차 = 기록·열람만(보상 없음, 수집률 보상은 2차). **다음 = ⑩ 지도+시계 나침반(마지막).** (→ [items.md §아이템 도감](items.md)) |
| 2026-07-11 | **갭 분석 통합 순서 구현 진행: ⑧ 완료(재정의).** ✅ ⑧ = 「시간 잔상」 **폐기** → **약탈자 회수**(사용자 결정). 사망 시 소지품(보안 제외)을 `ScavengerLoot` 대기열 캡처(삭제 X) → **다음 레이드 스폰 적 `GameTuning.scavengerCount`(3)마리를 약탈자 지정**(보랏빛 틴트로 식별, ②`LootContainer` 재사용)해 라운드로빈 분배 → 잡아 시체 뒤지면 회수. **한 번의 기회**(분배 시 대기열 비움, 못 챙기면 영구 소실). 세션 런타임(세이브 후속). `RaidManager.OnPlayerDeath` 캡처 · `EnemySpawner` 스폰 후 Distribute · `EnemyController.MakeScavenger` · 신규 `Combat/ScavengerLoot.cs`. F1 씬 탭 "잃은 물건 시뮬". **다음 = ⑨ 아이템 도감.** (→ [raid.md §약탈자 회수](raid.md)) |
| 2026-07-11 | **갭 분석 통합 순서 구현 진행: ⑦ 완료.** ✅ ⑦상인 재고 회전(A 「오늘의 입하」) — **1일 주기**(일일퀘 날짜 경계 `DailyQuestManager.Day()`, 사용자 선택). `ShopUI`가 날짜별 진열 = **고정**(`ShopData.fixedStock`, 기본 비움) + **회전 슬롯**(`GameTuning.shopRotationSlots`=4, `stock` 풀에서 희귀도 가중·`shopId+day` 시드 결정론적, 신규 0) + **수량제**(`stockBank`, **날짜 변경 시 재입고**). 평판 게이트(`RarityToRepTier`) 유지. `RefreshStockCells`가 `CurrentDayStock()` 순회로 교체. 라디오 확정입고·특수매물 2차. F1 "다음날 강제 회전". **다음 = ⑧시체 회수(시간 잔상).** (→ [economy.md §A 「오늘의 입하」](economy.md)) |
| 2026-07-11 | **갭 분석 통합 순서 구현 진행: ⑥ 완료.** ✅ ⑥투척물(돌 유인) — `Combat/ThrowSystem.cs` 셀프부트 싱글턴(PlayerNoise 패턴, executionOrder 100). **G키**(돌 보유·비안전구역·비모달)→조준 모드(**사거리 원 + 커서 착탄 마커**, 밖=경계 클램프)→**좌클릭** 착탄→`PlayerNoise.Pulse`로 착탄 소음→반경 내 적 조사. 우클릭/ESC 취소, 돌 1개 소모, 데미지 0. 신규 아이템 `Resources/Items/Misc/Stone`(`ItemData.isThrowable` 필드), 수치 `GameTuning`(throwRange 8/throwNoiseRadius 9/throwSpeed 10 거리비례), 조준 중 공격 차단(TopDownPlayer), **퀵슬롯 등록 가능**(숫자키/클릭=조준 진입), F1 "돌 5개 지급". 소음(⑤)이 적 조사를 이미 처리해 착탄 펄스만으로 유인 성립. **다음 = ⑦상인 재고 회전.** (→ [combat.md §투척물](combat.md)) |
| 2026-07-10 | **갭 분석 통합 순서 구현 진행: ①~⑤ 완료.** ✅ ①설정 메뉴 · ②적 시체 루팅(동적 격자+가방 파밍) · ③퀵슬롯(6칸, 드래그/클릭+숫자키) · ④무게 초과 페널티(3구간, 안전가옥 면제) · ⑤소음 시스템(`NoiseSystem`/`PlayerNoise`/`NoiseHUD` + `EnemyController.Investigate` 조사 상태 + 발밑 링·파문·HUD 미터 + `move_noise` 특성 개통, 인간 적만). 다음 = ⑥투척물(소음 위에 얹음). |
