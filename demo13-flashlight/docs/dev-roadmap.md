...
| 2026-07-10 | **갭 분석 통합 순서 구현 진행: ①~④ 완료.** ✅ ①설정 메뉴(`GameSettings`/`SettingsUI`) · ②적 시체 루팅(`EnemyController.BecomeCorpse` + 동적 격자 + 타르코프 가방 파밍) · ③퀵슬롯(6칸, 드래그/클릭+숫자키 등록·의료음식 제한) · ④무게 초과 페널티(GameTuning 6필드 → `PlayerInventory` 3구간 → `TopDownPlayer` 이속·스프린트·스태미너, 하드컷 외부 유입 게이트). 다음 = ⑤소음 시스템(+⑥투척물 동반). |
