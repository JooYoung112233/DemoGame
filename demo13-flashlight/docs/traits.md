...
> ## ⚠️ 2026-09-09 — 41종 → **11종**으로 축소
> 볼륨 축소([`scope-cut.md`](scope-cut.md) 5번). 남긴 기준은 하나: **코드가 실제로 읽는 effectKey를 가진 퍽만.**
> 41개 중 27개는 `effects` 리스트만 채워져 있고 런타임에 아무도 읽지 않는 "종이 퍽"이었다.
>
> **남은 11종** (긍정 8 · 부정 3)
>
> | 카테고리 | 퍽 | effectKey | T/PP |
> |---|---|---|:--:|
> | 전투 | 끈질긴 폐활량 | `stamina_max`·`stamina_regen` | T1 / 1 |
> | 전투 | 냉정한 손 | `groggy_buildup` | T1 / 1 |
> | 전투 | 받아넘기기 | `dodge_iframe`·`dodge_stamina_cost` | T2 / 2 |
> | 생존 | 불굴 | `low_hp_damage_taken` | T3 / 3 |
> | 회수 | 노새 | `weight_max` | T1 / 1 |
> | 회수 | 감정가 | `sell_price` | T2 / 2 |
> | 잠행 | 그림자 | `detect_radius` | T1 / 1 |
> | 사회 | 단골 | `buy_price` | T2 / 2 |
> | 부정 | 유리 어깨 | `damage_taken` | −2 |
> | 부정 | 허약한 위장 | `hunger_thirst_rate` | −1 |
> | 부정 | 악몽 | `sleep_recovery` | −1 |
>
> **비워진 카테고리 — ★현상(Anomaly) 7종 전부 삭제.** 세계관 고유 트리라 "정체성"으로 잡아 뒀지만
> 한 줄도 배선되지 않은 기획 단계였다. **§3.6에 기획은 그대로 남겨 뒀으니 재개통 시 1순위.**
> `TraitPanelUI`는 빈 카테고리를 자동으로 건너뛴다(코드 수정 불필요).
>
> **함께 죽은 것**: `move_noise`(소음 폐기), `pain_penalty`·`fracture_chance`·`bleed_duration`(부위별 의료 폐기),
> `search_speed`(컨테이너 수색 — 인벤 슬롯화에서 정리 예정). 이 키를 쓰던 퍽은 남길 이유가 없었다.
> **선행 정리**: 불굴의 선행이던 「강골」이 사라져 `prereqTraitId`를 비웠다(단독 T3).
>
> 아래 §3 트리 표는 **폐기분 포함 원안**이다 — 되살릴 때 참고용으로 남긴다.
...
## 2. 포인트(PP) 경제 〔1차 초안 · 공급 루트 2026-07-02 개정〕

| 항목 | 1차값 | 비고 |
|---|---|---|
| **캐릭터 레벨업** | **+1 PP** / 레벨 | **주 공급원** — 레이드 종료 보상 = **경험치**, 레벨업 시 PP 지급. 경험치 설계(2026-07-06 확정): ① 소스 = **킬(유닛별 expReward 합) + 탈출 성공 보너스 + 루팅 가치 기반** ② 사망/시간초과 = 벌던 XP **50%만**(킬분만, 탈출·루팅 보너스 없음) ③ 레벨 공식 = **선형** — 다음 레벨 필요 XP = 100×현재레벨(수치 GameTuning) |
| 회수꾼 평판 레벨업 | **+1 PP** / 등급 | 보조 공급원으로 강등(유지). **배선됨(2026-07-07)** — 평판 등급업 시 `GrantPP`(quests-region1 §9.2 구현 메모) |
| 안전구역 아르바이트(납품) | 소량 PP (가끔) | 주 보상은 돈 — economy.md §아르바이트 참조 |
...
### 2026-09-09 — 특성 41종 → 11종 축소 (볼륨 축소 5/6)
- **질문**: "볼륨이 큰데 큰 덩어리 중 뭘 자를까 — 특성 41종 축소 / 총기 / 투척물 / QA?"
- **결정**: **특성 축소.** 남기는 기준 = **코드가 실제로 읽는 `effectKey`를 가진 퍽만.**
- **결과**: 긍정 8 + 부정 3 = **11종**. SO 30개 삭제(`Resources/Data/Traits/`). 표는 문서 상단 참조.
- **근거**: 41개 중 27개가 `effects`만 채워진 **종이 퍽**이었다 — 찍어도 게임이 달라지지 않는다.
  퍽 개수가 아니라 "찍으면 체감되는가"가 시스템의 값어치고, 그 기준을 넘긴 게 11개뿐이었다.
- **손실**: ★현상 카테고리 7종 전멸 — 세계관 고유 트리라 정체성으로 잡았던 축이다.
  **§3.6 기획은 보존**했고, 배선할 여력이 생기면 여기가 1순위다.
- **연쇄**: 소음·부위별 의료 폐기로 `move_noise`/`pain_penalty`/`fracture_chance`/`bleed_duration`이
  죽으면서 해당 퍽도 함께 정리됐다. 「강골」 삭제로 「불굴」의 선행을 비웠다.
- 전체 맥락: [`scope-cut.md`](scope-cut.md)

### 2026-07-06 — 경험치/캐릭터 레벨 시스템 설계 확정 (PP 주 공급원 구체화)
- **질문**: ①XP 소스(킬+탈출+루팅 / 킬+탈출만 / 탈출만) ②사망·시간초과 시 벌던 XP(절반/전부/상실) ③레벨 공식·레벨당 PP(선형·+1 / 고정·+1 / 직접)?
- **결정**: ① **킬+탈출+루팅** — 킬=유닛별 `UnitStatData.expReward` 합, 탈출 성공=고정 보너스, 루팅=획득 아이템 가치 기반. ② 실패(사망/시간초과) = **킬 XP의 50%만**(탈출·루팅 보너스 없음 — "죽어도 배운다"). ③ **선형 증가**: 다음 레벨 필요 XP = 100×현재레벨, **레벨업마다 PP +1**. 수치는 전부 GameTuning(Control Panel) — 데모 도달 PP ≈ 4~6 목표에 맞춰 조정.
- **구현 방향**: `PlayerProgress`(싱글톤, 세이브 영속: level/xp) 신설, `RaidManager`가 레이드 중 킬 XP 누적(`EnemyController.OnDeath` 훅) → 종료 경로별 정산 → `PlayerProgress.GrantXp` → 레벨업 시 `TraitManager.GrantPP(1)` + 토스트. 레벨/XP HUD·캐릭터 패널 표시는 후속(TBD).
- **구현됨(2026-07-06, 그레이박스)**: 위 방향대로 + **루팅 가치 = 소지품 총가치(가방+주머니+보안+장비, sellPrice×스택)의 레이드 시작/종료 차액(순증가분)** — 픽업 경로(E키/클러스터/컨테이너 드래그)와 무관하게 전부 집계, 들고 간 장비는 자연 상쇄(리뷰 지적 반영: lootedItems 방식은 컨테이너 루팅 미집계 + 스택 참조 오염). XP 지급은 `SaveCheckpoints.RaidEnded()` 커밋 직전 = 레벨업 PP까지 같은 커밋에 원자 포함. 수치 5종 = GameTuning(balance.md §2).

### 2026-07-02 — PP 획득 루트 확정 (레이드=경험치 → 캐릭터 레벨업=PP)
- **질문**: PP 획득 루트 — ①레이드 종료 직접 지급(평판 지급 대체) ②둘 다 유지 ③레이드=경험치만, 캐릭터 레벨업 시 레벨당 PP(RPG 정석)?
- **결정**: **③ 레이드 종료 보상 = 경험치, 캐릭터 레벨업 시 레벨당 PP 지급.** §2 표의 주 공급원을 평판 레벨업 → **캐릭터 레벨업**으로 이동(평판 +1PP는 보조로 유지). 추가 소량 공급: 안전구역 아르바이트 반복 시 가끔(economy.md).
- **전제/TODO**: **경험치·캐릭터 레벨 시스템 신설 필요(미구현)** — 레이드 종료 시 경험치 산정 → 레벨업 → `TraitManager.GrantPP(N)`. 레벨당 PP 수치(N)·경험치 곡선 TBD.
