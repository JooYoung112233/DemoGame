...
### 2026-06-19 — 하이드아웃 업그레이드 다재료화 (재료 싱크 확장)

**정정**: 하이드아웃 업그레이드 시스템은 **이미 존재**한다(`HideoutModuleManager` — 8모듈 stash/quarters/workbench/medbench/cooking/generator/radio/dispatch, Lv1~10, 스크랩+재료 소비, `OnModuleUpgraded`, SaveManager `hideoutModules` 영속화, `HideoutUI` 비용 표시, `MainStash`가 stash 레벨로 격자 확장). 진짜 문제는 **레벨당 재료가 1종뿐**이었던 것(비용 헬퍼 `C(scrap,id,qty)`가 `mats`에 1종만 채움) → "재료 여러 개" 불가.

- **결정·구현**: 비용 테이블을 **다재료화**. `Cost.mats`(배열)·`HideoutUI`(foreach mats)는 이미 다재료 지원 → **헬퍼 `CM(scrap, params (id,qty)[])` 추가**하고 8모듈 × Lv1~3 비용을 **각 2~3종 재료 + 고티어(Lv3)는 루디(ruby_shard)**로 재작성. 스크랩 값은 유지. `tools/balance/hideout_modules.csv` 미러도 `materials`를 `a:5|b:4|c:1`(`|` 구분) 다재료로 동기화. Lv4~10 스케일 공식은 mats 전체에 적용돼 그대로 동작.
- **근거**: 구조·UI는 멀쩡 — **데이터(1종)만 병목**. 다재료화로 "재료 여러 개" 즉시 해소 + 재료 수요·레이드 동기↑, 신규 코드/재료 최소(헬퍼 1개). 루디 = Lv3 고티어 비용(gdd-core §5.3 마모 싱크 정합).
- **후속(미실행)**: 깊은 제작 레시피 추가(3+재료), 지역 루트 재료 스프레드 점검([region-loot.md](region-loot.md))은 별도 작업.
