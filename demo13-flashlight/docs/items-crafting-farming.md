...
### 2026-06-19 — 크래프팅 깊이 + 하이드아웃 재료 싱크 (분량 확장 기획)

현황: 재료 **12종**, 레시피 **15**(대부분 1~2재료). 시설은 있으나 **하이드아웃 업그레이드/재료 싱크가 없어** "재료 여러 개 쓰는" 깊이가 안 난다(사용자: "하이드아웃 재료 1개뿐 → 다재료 불가").

- **결정(핵심): 하이드아웃 시설 업그레이드 도입(타르코프식 재료 싱크).** 작업대/의료대/조리대/보관함 **Lv↑ = 여러 재료 + 루디** 소모. → 재료 다수 수요·레이드 동기 발생. 그레이박스 = 레시피로 표현(업그레이드 전용 station 추가 또는 별도 업그레이드 데이터 — 구현 판단 TBD).
- **결정: 깊은 레시피 추가.** 3+재료 조합·중급 재료 단계화(예: 부품→정제부품). 기존 12재료 우선 활용, **신규 재료 최소**.
- **결정: 지역 루트 재료 스프레드 보강.** 다재료 레시피가 실제로 모이도록 [region-loot.md](region-loot.md) 재료 분포 점검.
- **근거**: 구조(`RecipeIngredient[]`)는 다재료 지원 — 문제는 **재료를 쓸 싱크(업그레이드)와 깊은 레시피가 없는 것**. 싱크 추가가 핵심. 루디 = 고티어 업그레이드 비용(gdd-core §5.3 마모 싱크와 정합). **실행은 content-author/balance-tuner로(스코프 확정 후).**
