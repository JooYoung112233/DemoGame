...
## 2026-09-13 — 만화풍 텍스처에 맞춘 조명·셰이더 정리

- 날짜: 2026-09-13
- 질문/요청: “지금 조명이나 셰이더 같은 게 텍스처 만화랑 조금 안 어울린다. 수정해줘.”
- 사용자 결정: 승인된 B 만화풍 텍스처에 맞게 실제 조명과 셰이더 표현을 조정한다.
- 실행 기준: 텍스처의 진한 선·그려 넣은 그림자·고유 색을 살리고 사실적인 표면 반사, 절차적 얼룩, 과한 탈색과 번짐을 줄인다. 기존 낮밤 구분·손전등·실내 조명·실시간 그림자는 유지하며 62° 마을·캐릭터·실내의 낮밤 결과를 비교 검수한다. 구체 수치는 자체 비교 후 기록하며 최종 미감 승인은 사용자에게 별도다.
- 적용 완료: `BRB/GameLit`와 `BRB/GroundFadeLit`의 기본 `_ComicLighting=1`. 무광 확산광으로 텍스처 색·선을 유지하고 절차적 얼룩/그늘 탈색/림을 끈다. 노멀 강도 상한 0.12, AO 강도 상한 0.35. 발광으로 어둠을 보정하지 않으며 URP 주광·추가광·그림자·Forward+·안개 경로를 유지한다. 값 0은 기존 룩 조정용이다.
- 만화 텍스처가 연결된 불투명 URP Lit 재질 55개를 GameLit으로 전환했다. 캐릭터의 기존 GameLit과 지면 페이드 셰이더에도 새 기본값이 적용된다. 투명/VFX 셰이더는 전환하지 않는다. 목록: `ArtWork/ComicLighting62/Applied.json`.
- 저장 수치: 낮 주광 0.82 / RGB(1,.98,.94), 주변광 (.25,.28,.32). 밤 주광 0.30 / (.63,.70,.88), 주변광 (.13,.16,.23), 안개 (.06,.08,.12) / 밀도 .016. 태양 각도는 기존 58° 유지. 가로등 낮/밤 2.5/9, 현관등 .65/2.8·범위 4.5, 실내 전구 1.8. 가로등·현관등 색 (1,.87,.69), 실내 전구 (1,.92,.80). WeatherData·Safehouse 씬·현관 프리팹에 저장했다.
- 후처리: Neutral 유지, 노출 +.05, 대비 +6, 채도 +5, Bloom .04/문턱 1.4/scatter .35, Vignette .10, Film Grain 0. `PlayerRigVolume3D`와 프로파일 재생성 코드에 동일하게 반영했다.
- 자체 검수: 저장 후 플레이 재실행, 62° 마을/플레이어/NPC/전당포 실내 낮밤 10컷, 실제 하이드아웃 카메라, 야간 착용등 유무 비교. 광원 7개의 낮밤 전환 및 재활성화 14건 통과. 두 셰이더 진단 0, 마을 오류 셰이더/미전환 Comic URP Lit 0. 전체 레이드/기상/플랫폼 빌드 QA는 이번 범위에 포함하지 않는다. 증빙: `ArtWork/ComicLighting62/README.md`.

## 2026-09-12 — 기존 모델 전체 만화풍 텍스처 리메이크

- 사용자 결정: “좋다 이 화풍으로 지금 있는 모델 전체 적용 텍스쳐 리메이크 하고 그다음 마을에 프랍 작업 더 진행하자 너무 밋밋해”. 실제 연결된 플레이어 B의 화풍을 승인하고 **기존 모델 전체 텍스처 리메이크 → 마을 프랍 보강** 순서를 요청했다.
- 실행 기준: 캐릭터/NPC/적/건물/환경 프랍/장비의 현용 재질과 모델 의존성을 조사하고 진한 표면 선·명확한 색면 그림자로 통일한다. 원본은 보존하고 UI·VFX는 모델 텍스처 대상과 구분한다. 실제 연결 목록과 미처리 사유를 남겨 일부만 바꾸고 전체 완료로 표시하지 않는다.
- 적용 완료: NPC 4명·현용 SimpleBandit·BanditClub 아틀라스 6종과 환경 공용 표면 8종을 제작했다. 독립 재질 136개, FBX 내부 슬롯 114개, 씬 Renderer의 직접 재질 연결 70개를 저장했다. 플레이어 B는 승인 기준 그대로 유지한다. 저작용 구형 Bandit01/SimpleHero 원본·UI/VFX·발광/지도선/풀잎 등 의도적인 단색은 보존한다. **모델 수와 재질/슬롯 수를 혼동하지 않는다.** 전체 경로·제외 사유: `ArtWork/WorldComic62/Unity/MaterialApply.json`, `Coverage.json`.
- 자체 검수 완료: 캐릭터 4방향/62°, 적·무기·하이드아웃 실내, 마을 8구역. 과한 나뭇결과 균열 반복은 재생성했고 도로의 중복 UV 반복은 0.25로 수정했다. 원본 이미지·모델·UV·리그·애니메이션·셰이더 소스 보존. Unity MCP 임포트/저장 후 플레이 재실행, 오류 셰이더/누락 스크립트 0. 새 결과의 최종 미감 승인은 사용자에게 별도다.
- 검수 범위: 낮 실제 씬과 야간 WeatherData 조명 조건에서 표면을 확인했다. 야간 캐릭터 근접 컷은 검사 카메라의 near clip/안개 간섭을 피하려고 안개를 제외했다. 전 시간대 게임 플레이 QA 완료로 표현하지 않는다. 증빙과 프롬프트: `ArtWork/WorldComic62/README.md`. 이 후 마을 프랍 38개 배치까지 진행했다([safehouse.md](safehouse.md)).

## 2026-09-12 — B 만화풍·텍스처 그림자 허용

- 질문/선택: A/B/C 텍스처 시안 중 **B의 진한 만화풍 선**을 선택했다. 이어 “텍스쳐네 그림자 넣어도 된다 개성이니까 만화풍이잖아”라고 결정했다.
- 현재 기준: 진한 표면 선과 경계가 명확한 2~3단 색면 그림자를 텍스처 자체에 그린다. 아래 손그림 기준의 '현재 조명을 텍스처에 굽지 않는다' 제한은 만화풍 그림자에는 적용하지 않는다. 주름·접힘과 면 구분에 맞춘 의도적인 명암을 사용하고 게임 조명과 겹친 가독성을 검수한다.
- 적용 순서: 플레이어 우선. 기존 셰이더·모델·UV·리그를 유지하며 다른 캐릭터/환경은 후속이다. 화면 외곽선 효과는 이번 텍스처 작업에 포함하지 않는다.
...
- 2026-09-12: B 진한 만화풍 선과 텍스처에 그린 색면 그림자를 사용자 선택으로 확정. 플레이어 두 BaseMap에 먼저 저장·연결, 다방향/62°/게임 화면 크기 검수. 기존 셰이더 유지.
...
- 2026-09-12: 승인된 Comic B를 현용 캐릭터·장비·환경에 확장. Base Color 14종, 기존 재질 136개/FBX 슬롯 114개/씬 Renderer 연결 70개 저장 및 자체 검수. 원본/단색 의도/저작용 구형 모델 보존 범위 명시.

- 2026-09-13: Comic B용 무광 조명 기본값, 불투명 재질 55개 전환, 낮밤·실내 광원 및 후처리 저장. 62° 비교/실제 하이드아웃/착용등 반응과 14건 광원 페이즈 검증 완료.
## 2026-09-13 — 휘두르기 중 머리에 남는 착용등

- 사용자 피드백: “휘두를 때 뒤통수에 라이트가 박혀 있는 게 보인다. 수정해 달라.”
- 확인 질문: 전구 모양 에디터 아이콘 / 실제 빛·부품 중 어느 쪽인지. 회신 전 실제 광원 배치를 검사했다. Game View 기즈모는 꺼져 있으며 `PlayerLamp3D`는 Player 루트의 Y=1.15m에 고정돼 있었다.
- 구현 판단: 착용등은 기존 옷 부착 방향대로 애니메이션 `Chest`를 따라가며, 가슴 회전 기준 (.12,-.04,.22)m 앞섶에 배치한다. FBX 뼈 스케일 100은 오프셋에 곱하지 않는다. 광원 위치를 갱신한 뒤 주변광·몸 글로우를 배치한다. 등급·낮밤 밝기·빔 방향 정책은 유지한다.
- Unity MCP 검증 11/11 통과: 대기·걷기·세 공격의 가슴 추적과 머리/모자 바운드 관통 0회. 실제 애니메이션을 BakeMesh(true)로 샘플링하여 광원 위치를 비교했으며, 야간 밝기를 강제한 공격 게임 화면을 자체 검수한 뒤 기존 낮밤 상태로 복원했다. `ArtWork/LampClearance/Validation.json`, 콘솔 오류 0건. 전구 아이콘 여부에 대한 사용자 답변과 최종 화면 확인은 별도다.
