...
|---|---|---|
| 2026-09-11 | "그 쿼터뷰가 아닌데 아직" — 같은 자리 4장 비교: ① 55°·정북(지금) ② 40°·정북 ③ 대각선 45°·45° ④ 대각선 35°·45° | **③ 대각선(방위 45°) · 45° 내려봄** — 아이소식 쿼터뷰. 이동도 화면 기준(W = 화면 위)으로 |
...
- 카메라: `CameraFollow.viewPitch/viewYaw`(45/45)가 진실 — Start에서 돌린 뒤 추적 오프셋을 잡는다. 이동·패드 조준 스틱은 `TopDownPlayer.CameraRelative`로 화면 기준.
...
- **아직 결정·작업이 남은 것**: 미니맵·지도(`NavigationHUD`·`RaidMapUI`)는 정북 고정이라 W를 누르면 표시가 대각선으로 간다(돌릴지 결정 필요) ·
  마을 상점 간판이 55°로 굳어 있다(`Safehouse3DLayout`, 다른 세션 마을 작업) · 북·동쪽 면의 문은 카메라 반대편이라 안 보인다(맵 배치 점검).
- 분위기 = `Resources/Data/WeatherData.asset`(낮/밤 두 페이즈 — 밤 페이즈를 "저녁"으로):

| | 이전 | 지금 |
|---|---|---|
| 낮 태양 | 세기 1.0 · 흰빛(1, .95, .9) · 고도 50° | 세기 **0.8** · **주황**(1, .78, .55) · 고도 **38°**(늦은 오후) |
| 낮 앰비언트 | 회청(.35, .35, .40) | **따뜻하고 어둡게**(.30, .25, .20) |
| 밤 빛 | 세기 0.18 · 청색(.55, .62, .85) | 세기 **0.22** · **보라**(.62, .50, .85) |
| 밤 앰비언트 | (.10, .11, .15) | **보라**(.13, .10, .19) |
| 밤 안개 | 거의 검정(.02, .02, .03) | **보랏빛 어둠**(.06, .04, .09), 밀도 0.04 그대로 |
