...
## 2026-09-12 — 손그림풍 전체 텍스처 기준 채택

- 사용자 평가·결정: 플레이어 적용본에 “좋긴한데 약간 개성 더 넣었으면 좋겠네 이 풍으로 우리모든 텍스쳐 할꺼니까”. **현재 플레이어의 손그림 색면·그려 넣은 주름·봉제·국소 마모를 전체 텍스처의 공통 방향으로 채택**한다. 캐릭터와 환경 모두 대상이며, 기존 텍스처가 이미 전부 교체됐다는 뜻은 아니다.
- 다음 실행: 플레이어의 현 팔레트와 모델·셰이더를 유지하면서 작은 고유 표식과 비대칭 수선 흔적을 보완한다. 모든 면에 무늬를 늘리지 않고 62°에서 읽히는 몇 군데에 집중한다. 세부 표식·배치의 최종 승인은 별도다.
- 공통 제작 기준: 큰 색면으로 재질을 구분하고, 주름·결·봉제선은 형태와 용도에 맞춰 그린다. 사진 같은 잔노이즈와 균일한 얼룩 반복을 피하고 넓은 조용한 면을 남긴다. 현재 조명을 텍스처에 굽지 않는다. 기존 UV와 원본 파일을 보존하고 실제 Unity 다방향/게임 시점에서 검수한다.

## 2026-09-12 — 애니풍 텍스처 방향 시안

- 질문·사용자 결정: “다음 진행하자”, “텍스쳐를 약간 애니풍으로 하면 좋지 않을까? 셰이더 말고 텍스쳐 자체… 감성이 필요”. 셰이더 변경 대신 텍스처 표현을 다듬는 방향으로 진행한다. 세부 스타일의 최종 외형 승인은 아직 받지 않았다.
- 첫 시안 범위: 방금 적용한 하이드아웃 입구의 포장·흙 표면. 기존 원화/차분한 팔레트를 대조하고 사진 같은 잔노이즈를 낮춰 넓은 손그림 색면·붓결·국소 마모를 만든다. 원본 텍스처는 보존하고 별도 후보를 같은 모델·카메라·조명·셰이더에서 비교 검수한다. 전 마을·캐릭터 일괄 교체는 시안 확인 후 후속이다.
- 시안 제작·자체 검수 완료: 내장 image_gen의 콘크리트/흙 Base Color 2종, 후보 재질 5개와 별도 프리팹을 `Assets/Art/Environments/PaintedSurface62`에 저장했다. 초기 사실적인 박리 표현과 Unity에서 위장무늬처럼 반복되던 얼룩을 수정했다. 콘크리트 대비를 낮추고 Base Map 반복 배율을 .4로 조정, Mirror 래핑으로 가장자리 색 단절을 방지한다. 원본 노멀/마스크 파일은 유지하되 공통 UV 때문에 샘플링 크기는 함께 변한다.
- 상태: 셰이더·조명 변경 없음. 낮밤 62° A/B 자체 검수와 MCP 경고·오류 0 확인. **현재 마을에서 런타임 미리보기만 켜짐 / 원본 씬·재질은 보존 / 씬 재로드 시 원래 모습 복원 / 최종 스타일 승인 별도**. 재현·프롬프트·증빙은 `ArtWork/PaintedSurface62/README.md` 참조. 콘크리트의 변화는 절제되어 있으며 전체 감성 판단을 위한 벽·목재 시안은 후속이다.
...
- 수정: 안개 강도가 0이면 렌더러 초기화를 요청하지 않는다. 실제 안개가 필요할 때만 셰이더를 확인하고, 누락 결과를 캐시해 컴포넌트 활성화 기간 중 재검색·경고는 한 번으로 제한한다. 셰이더 복구 후 컴포넌트 재활성화로 재시도할 수 있다. 비활성화 시 생성 리소스를 정리하며 영속화는 `HierarchyFolder.Persist`를 따른다.
- 추가 수정: `VisionDarkness`는 제거된 전용 셰이더가 없는 상태에서 자동으로 빈 메시·렌더러를 만들지 않는다. 명시적으로 씬에 배치한 컴포넌트의 누락 진단은 유지한다.
- 범위: 2026-09-10에 제거된 안개·시야 어둠 셰이더 자체를 복구한 것은 아니다. 현재 조명·시야 판정·현상 게임플레이는 그대로이며 경고 필터를 숨기지 않는다. 검증 자료는 `ArtWork/StartupWarnings`.
- 검증: MCP 컴파일 통과. 안개 강제 요청 180프레임 동안 누락 진단 1회·추가 검색 없음·오류 렌더러 생성 없음 확인. 이후 새 Systems 실행→저장 로드→Safehouse→Hideout 경로에서 경고 0·오류 0건, 하이드아웃 강도 0에서는 셰이더 검색 자체가 발생하지 않았다. 저장 재귀 경고 3건 수정은 [save.md](save.md) 참고.
...
- 2026-09-12: 사용자 “이 풍으로 우리모든 텍스쳐”에 따라 현재 플레이어 손그림풍을 전체 텍스처 기준으로 채택. 팔레트/형태를 읽히게 하는 색면·주름·봉제·국소 마모가 기준이며 일괄 교체 완료를 의미하지 않는다. 플레이어 개성 보완 V2 연결 완료.

- 2026-09-12: 사용자 애니풍 텍스처 요청에 따라 콘크리트·흙 2종 시안 제작. 반복 얼룩 수정 후 62° 낮밤 검수, 별도 재질·프리팹 및 현재 플레이 미리보기로 보존. 원본 씬·셰이더 유지, 최종 승인 별도.
...
**아직 셰이더가 없어 꺼져 있는 기능**(2026-09-10 백지화 때 사라진 것): 이상현상 안개(`BRB/AnomalyFog` — 2026-09-12부터 실제 효과 요청 시 활성화 기간당 한 번만 진단),
