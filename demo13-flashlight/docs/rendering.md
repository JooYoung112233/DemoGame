...
- 사용자 선택·결정: 우선순위에 대한 의견 요청이며 셰이더 구현 착수·수치 변경은 아직 확정하지 않았다.
...
- 제안: 표현을 맞추는 작업을 다음 우선순위로 삼되, 먼저 동일 시간대의 조명·노출·색보정을 기준화하고 피부/천/목재/금속의 재질 차이를 조정한다. 그 결과 부족한 부분에 기존 `BRB/GameLit` 개선을 적용한다. 현재 NPC Mask는 셰이더 슬롯 부재로 미연결이므로 재질별 거칠기/금속성 표현이 필요할 때 지원을 검토한다. 새 모델 대량 제작 전에 전당포 실내와 야외 NPC 한 구역에서 기준을 잡는 것을 권장한다.
