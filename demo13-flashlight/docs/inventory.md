...
- **할당**: 인벤 UI에서 **드래그로 원하는 슬롯에 직접 놓기**(지정 등록·교체, `TryAssignAtScreenPoint` — 아이템 이동이 아니라 id 등록, 아이템은 원위치 유지). 우클릭 "퀵슬롯"(첫 빈 칸 토글)도 유지. **의료·음식(Medical/Consumable)만 등록 가능**(`IsAssignable` — 드래그·우클릭 공통 게이트).
