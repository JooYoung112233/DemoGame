...
## 퀵슬롯 — 하단 핫바 4칸 (2026-07-10 확정, 구현 전)

> 질문: 레이드 중 소모품 사용 UX는. **결정: 하단 핫바 4칸(1~4키) + 인벤 드래그 할당 + 즉시 사용.**

- **구성**: 화면 하단 핫바 **4칸**, 단축키 **1~4**.
- **할당**: 인벤 UI에서 **드래그**로 소모품(의료·음식) 할당. (기존 QuickSlotBar 6칸·우클릭 등록 방식(2026-06-19 구현)을 **대체**.)
- **사용**: 1차 = **즉시 사용**. 의료 캐스트 시간(시전)은 2차 — [medical.md](medical.md) 규칙 따름.
- **표시**: 레이드/거점 **공통 표시**.
- **영속**: 슬롯 할당은 **세이브에 저장**(기존 런타임 전용 → 영속화).
