...
## 무게 시스템 — 초과 페널티 (2026-07-10 확정, 구현 전)

> **하드컷 폐지 → 타르코프식 3구간 페널티 곡선.** "하나 더 주울까?"의 핵심 감정을 무게가 만들게 한다.

기준 = `MaxWeight`(기본 30kg × 특성 `weight_max` 배율 — **특성이 구간 기준을 그대로 밀어올림**). 현재 무게/MaxWeight 비율로 구간 판정:
...
| 하드컷 | 130% 이상 | **더 못 담음** (배치/줍기 차단) |

- 수치는 **전부 GameTuning 필드화** (1차 초안, 플레이 조정 전제): `overweightSoftPct=1.0` / `overweightSprintBlockPct` / `overweightSlow1=0.15` / `overweightSlow2=0.30` / `overweightHardPct=1.3`. [→ balance.md §2](balance.md)
- 구현 순서: 갭 분석 채택 5종 중 **2번째** (①설정 메뉴 → ②무게 페널티 → ③상인 재고 회전 → ④도감 → ⑤투척물 — dev-roadmap.md 2026-07-10).
