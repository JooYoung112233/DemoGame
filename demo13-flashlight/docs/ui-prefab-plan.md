...
> **룩의 진실 = ⭐[`ui-reference.md`](ui-reference.md)** (2026-07-29 확정) — 캐논 **이중 언어**(종이=문서형 / 프레임=기능형) + 28패널 배분표. 어느 패널에 어느 스프라이트를 입힐지는 §5 매핑이 아니라 그쪽 배분표를 따른다.
> **발주 스펙 = [`ui-resources.md`](ui-resources.md) + [`ui-asset-list.csv`](ui-asset-list.csv).** 이 단계 진입 전제 = 그쪽 P0(시안 13장 재납품 + 버튼 상태 + 게이지 + 슬롯 하이라이트) 납품.
> ⚠️ 2026-07-29 실측: `Resources/UI/*.prefab` = **28개**(본 문서 §4-A의 "26종"은 `CodexUI`/`SettingsUI` 합류 전 수치 — 갱신 필요). 그리고 기존 시안 스프라이트 13장은 목업(1366×791) 1:1 컷이라 **1920 기준 대비 1.41× 업스케일 중 → 그대로 입히면 종이 질감이 뭉개진다**(`ui-resources.md` §1).
