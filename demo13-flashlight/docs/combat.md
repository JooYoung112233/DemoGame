...
## 2026-09-13 — 방망이 그립·공격 선후딜 검수 요청

- 질문/피드백: “방망이가 손에 안 맞고 모션 선·후딜이 길다. 살펴봐 달라.” 별도 선택지 없음.
- 사용자 결정: 손과 손잡이 정렬을 수정하고 공격 타격 시점·선후딜을 실제 모션과 비교하여 조작감을 개선한다. 우선 방망이의 대기·걷기·공격 자세와 실제 적용 공격 데이터를 확인한다. 수치 조정은 검수 후 이 항목에 확정값을 기록한다.
- 진단: 양손 소켓은 방망이 축 x=z=0, 왼손은 y=-0.15m로 모션 내내 맞지만 실제 메시 축은 x=-0.10m/z=-0.025m로 빗나가 있다. 프리팹의 방망이 로컬 위치를 메시 축 오프셋만큼 보정한다. 기존 모델·만화풍 텍스처·양손 리그를 유지하는 연결 수정이다.
- 반영 수치(구현 판단): 48프레임과 타격 프레임 17~21을 유지하고 약공 30→60fps(1.6→0.8초), 강공 30→48fps(1.6→1.0초), 풀차지 30→40fps(1.6→1.2초)로 재생한다. 약공 타격 시작 0.567→0.283초, 타격 종료 이후 복귀 0.867→0.433초. 원본 클립과 타격 판정이 같은 비율로 빨라진다. 차징 시간·피해·스태미너·타격 범위는 기존 데이터 유지.
- Unity MCP 플레이 검증: 실제 공격 길이 0.8008/1.0000/1.2018초, 최초 타격 활성 0.2845/0.3566/0.4259초. 세 공격의 애니메이션·판정 진행 오차 최대 1.23%, 양손 축 오차 최대 0.91mm. 대기·걷기 그립과 장비 UI 포함 23/23 통과. 62° 게임 화면 및 20°/62° 스킨 메시 샘플 자체 검수 완료. 사용자 최종 조작감 승인은 별도.

## 2026-09-13 — 권총과 근접무기 장비 연결

- 질문: 첫 무기 종류(권총 / 권총과 근접무기 / 기존 무기) → 사용자: **권총과 근접무기**.
- 현용 9mm 권총과 양손 야구방망이를 사용한다. 신규 모델 제작 없이 기존 손 장착/모션에 연결하고 해당 모델에서 인벤 아이콘을 렌더했다.
- 퀵슬롯은 실제 아이템을 휴대 격자↔장비 또는 보조↔주무기 간 교환한다. 전투는 손에 든 주무기를 읽는다. 인벤토리에 같은 실물이 중복으로 남지 않는다.
- 장전 중 다른 무기로 바꾸면 장전을 취소한다. 새 탄창이 비운 칸에 이전 탄창과 잔탄을 돌려놓는다. 저장/보관 정책과 첫 지급 구성은 [inventory.md](inventory.md) 기준.

## 2026-09-12 — 걷기 동작의 급한 느낌 완화

- 사용자 피드백: “걷는 게 너무 경박스럽다.” 별도 선택지 없음. 직전 걷기 2.67배 재생이 지나치게 급한 것으로 보고 걷기 템포를 낮춘다.
- 결정·반영 기준: 이동 속도 1.2m/s는 유지하고 `playerStat.motions[walk].animSpeed`를 1→0.5로 조정해 실제 걷기 재생 속도를 약 2.67→1.33배로 낮춘다. 달리기·앉아 이동·공격 재생 속도는 유지한다. 원본 클립의 보폭은 그대로이므로 발 접지의 완전한 일치까지 보장하는 변경은 아니다.
- 검증: Unity MCP 재컴파일·StatDB 저장·재실행 후 걷기 1.19934m/s/Animator 1.33333, 달리기 3.60108m/s/Animator 1.56522를 확인했다. 경고 0건. 모션 배속 검증 완료이며 사용자 최종 체감 승인은 별도다. 자료 `ArtWork/CalmerWalk/after.json`.

## 2026-09-12 — 맵 규모에 맞춘 이동 속도 재조정

- 사용자 요청·결정: “걷기, 달리기 다 좋은데 맵 크기에 비해 너무 느리다. 애니 속도를 늘리든 해서 평균을 맞추고 대각선 이동이 빠른 버그도 수정.” 별도 선택지 없음.
- 기존의 “모션 원본 배속 그대로” 제약을 변경하고, 이동 속도와 애니메이션 재생 속도를 함께 조정한다. 최종 수치는 실제 맵 이동 및 8방향 거리 측정 후 아래에 기록한다. 대각선 입력을 더해도 직선보다 최대 이동 속도가 높아지지 않으며 아날로그 약입력은 보존한다.
- 반영값: 걷기 0.45→1.2m/s, 달리기 2.295→3.6m/s, 앉아 이동 0.225→0.6m/s. 100m 이동 기준 걷기 약 222→83초, 달리기 약 44→28초. 걷기/달리기 발 접지 기준 0.45/2.3m/s는 유지하고 재생 배속을 실제 속도에 맞춘다. 앉기 상태는 `crouch`/`crouch_walk` 모션 수치를 사용한다(전용 클립은 여전히 없어 Walk 재사용).
- 입력 보강: 키 조합을 카메라 변환 전에 길이 1로 제한하고, 카메라 투영 축을 직교 단위축으로 만들어 회전/기울임에서도 크기를 유지한다. `WorldToInput` 역시 같은 축을 사용한다. 이전 빌드에도 변환 후 ClampMagnitude는 있었으며 현재 카메라에서 1.414배 증가를 재현한 것으로 기록하지 않는다.
- 검증 완료: Unity MCP 재실행 후 실제 Rigidbody 거리 측정. 걷기 1.19934~1.19980, 달리기 3.59941~3.60108, 앉아 이동 0.59990~0.60120m/s. 각 모드 8방향+길이 0.5 아날로그 입력 총 27건 통과(허용 오차 0.025m/s). Animator 속도 2.6667/1.5652/1.3333 확인. 마을→하이드아웃→마을 전환 후 이동·물리·상호작용 상태 복구, 경고 0·오류 0. 자료 `ArtWork/MovementPace/validated-movement.json`.
...
- **현재값(2026-09-12 후속 피드백):** 걷기 **1.2m/s**, 달리기 **3.6m/s**(×3), 앉아 이동 **0.6m/s**(×0.5). 맵 크기에 비해 느리다는 사용자 요청으로 원본 배속 고정 제약을 해제했다. 실제 속도 연동과 걷기 전용 배율 0.5로 걷기 약 1.33배·달리기 약 1.57배·앉아 이동 약 1.33배 재생, 애니 배속 상한 2.7.
...
- **2026-09-11 이전 결정 기록 (이동속도 재산정, 이후 맵 규모 피드백으로 배속 허용):** 질문 "캐릭터 이동속도가 너무 빠르다 — 걷기 `moveSpeed`(현재 4 m/s)를 3.5 / 3.0 / 2.8 중 얼마로? 달리기 배율(×1.6)은 유지 / ×1.4 / ×1.8?" → 사용자: **"실제 캐릭터 모션을 보고 파악해줘 — 달리기·걷기 움직이는 모든 모션들"**, 달리기 배율은 선호 없음. 즉 속도 수치를 임의로 고르지 않고 **게임에 연결된 실제 이동 모션(걷기·달리기·앉아 걷기 등)의 발 접지 속도에 맞춰 산정**한다.
  - 측정 (Unity MCP, `SimpleHero.controller` 클립을 120프레임 샘플링해 발이 땅에 붙은 구간의 발 이동 속도, 모델 키 1.91 m·게임 배율 1): **Walk 0.45 m/s**(1.2초 루프, SwordWalk·BatWalk 동일) / **Run 약 2.3 m/s**(0.8초 루프, 좌 2.45·우 2.21 평균) / 앉아 걷기 전용 클립 없음(Walk 사용). 기존 게임 속도는 걷기 4.0(클립의 8.8배)·달리기 6.4(2.8배)였고 `animCadenceMatchesSpeed`가 꺼져 있어 발이 미끄러졌다.
  - 후속 질문 "측정값을 어떻게 반영할까? — 모션 원본 그대로(걷기 0.45·달리기 2.3) / 모션 배속 조금 허용(0.9·2.8) / 기본 이동을 달리기 모션으로(2.6·3.5)" → 사용자: **모션 원본 그대로**. 걷기가 적(1.9~2.5 m/s)보다 느려 걸어서는 거리를 벌릴 수 없다는 점을 알린 뒤 선택했다(2026-07-11 "적보다 빨라야 카이팅" 근거의 4 m/s를 대체).
  - **이전 적용 수치(현재값 아님):** `playerStat.moveSpeed` 4 → **0.45** · `sprintSpeedMultiplier` 1.6 → **5.1**(달리기 ≈2.3 m/s) · 앉아 걷기 = 0.45×0.5 ≈ 0.23 m/s(배율 그대로) · `PlayerRig` `animCadenceMatchesSpeed` 0 → **1** · `walkClipSpeed` 0.34 → **0.45** · `runClipSpeed` 0.52 → **2.3**(측정값).
- 속도 = 이동속도 × `sprintSpeedMultiplier`(현재 3). 초당 `sprintStaminaCost`(12) 소모, `sprintMinStamina`(10) 미만이면 불가.
...
- 2026-09-12: 원본 모션 기준 속도를 StatDB·PlayerRig·Systems에 실제 적용. 실측 중 대각선이 1.414배 빨라지는 기존 Vector2 속성 복사본 정규화 버그를 발견해 `ClampMagnitude` 결과 대입으로 수정. Unity MCP 이동 거리 회귀 검증은 `ArtWork/HideoutUIRepair/SpeedRegression.json`에 기록.

| 날짜 | 내용 |
|---|---|
| **2026-09-11** | **이동속도 재산정 방식 결정 (§달리기).** 사용자 "캐릭터 이동속도 너무 빠르다". 질문: 걷기 3.5/3.0/2.8 m/s · 달리기 배율 유지/1.4/1.8 → 사용자: **"실제 캐릭터 모션을 보고 파악해줘 — 달리기·걷기 움직이는 모든 모션들"**(배율 선호 없음). 측정: Walk 0.45 / Run 약 2.3 m/s(앉아 걷기 클립 없음). 후속 질문(원본 그대로 / 배속 조금 허용 / 기본 이동을 Run으로) → 사용자: **모션 원본 그대로**. 확정: `moveSpeed` 4→**0.45**, `sprintSpeedMultiplier` 1.6→**5.1**, `animCadenceMatchesSpeed` 0→**1**, `walkClipSpeed` 0.34→**0.45**, `runClipSpeed` 0.52→**2.3**. 걷기가 적(1.9~2.5)보다 느려진다는 점은 사용자 인지 후 결정. |
