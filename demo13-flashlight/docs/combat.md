...
- **2026-09-12 적용 요청:** 사용자 “잠만 속도가 그대로인데?” → 검수만 하고 미반영된 아래의 **원본 모션 그대로** 결정을 실제 StatDB·PlayerRig에 반영한다. 걷기 0.45m/s, 달리기 2.295m/s, 앉아 이동 0.225m/s, 모션 속도 연동 활성화. 새 속도 선택을 요구하지 않고 기존 확정값을 적용한다.
...
- **2026-09-11 사용자 결정 (이동속도 재산정, 2026-09-12 반영 완료):** 질문 "캐릭터 이동속도가 너무 빠르다 — 걷기 `moveSpeed`(현재 4 m/s)를 3.5 / 3.0 / 2.8 중 얼마로? 달리기 배율(×1.6)은 유지 / ×1.4 / ×1.8?" → 사용자: **"실제 캐릭터 모션을 보고 파악해줘 — 달리기·걷기 움직이는 모든 모션들"**, 달리기 배율은 선호 없음. 즉 속도 수치를 임의로 고르지 않고 **게임에 연결된 실제 이동 모션(걷기·달리기·앉아 걷기 등)의 발 접지 속도에 맞춰 산정**한다.
  ...
  - **확정 수치:** `playerStat.moveSpeed` 4 → **0.45** · `sprintSpeedMultiplier` 1.6 → **5.1**(달리기 ≈2.3 m/s) · 앉아 걷기 = 0.45×0.5 ≈ 0.23 m/s(배율 그대로) · `PlayerRig` `animCadenceMatchesSpeed` 0 → **1** · `walkClipSpeed` 0.34 → **0.45** · `runClipSpeed` 0.52 → **2.3**(측정값).
- 속도 = 이동속도 × `sprintSpeedMultiplier`(현재 5.1). 초당 `sprintStaminaCost`(12) 소모, `sprintMinStamina`(10) 미만이면 불가.
