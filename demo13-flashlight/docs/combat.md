...
- 빠르고 스태미너 소모 적음
- 그로기 수치 낮음
- 수치: 8/9/12 피해, 6/6/8 스태미너, 14/16/20 그로기(2026-09-11 그로기 조정 — 변경 로그)

### 강공격 (우클릭 홀드 → 릴리즈) — 근접 무기일 때만(총을 들면 우클릭 = 조준)
- 차징 시스템 (최소 0.6초 → 풀차지 1.5초)
- 느리지만 높은 피해 + 높은 그로기
- 적 예비동작 중 적중 시 공격 캔슬
- 수치: 20(일반)/32(풀차지) 피해, 22/35 스태미너, 32/58 그로기(2026-09-11 그로기 조정)
...
### 적 공격 예고 + 캔슬
- 적 공격 전 예비동작 — **유닛별 값**(StatDB `attackWindup`, 아래 적 유닛 표. 2026-09-12 문서를 실제 값에 맞춤)
...
| 날짜 | 내용 |
|---|---|
| **2026-09-12** | **적 예비동작 문서 교정.** 질문: 문서는 0.8초, StatDB는 유닛별 0.7~1.15초 — 문서를 유닛별 값으로 / 전부 0.8초로 통일. **결정: 문서를 유닛별 값으로**(사용자 선택). 게임 값은 그대로. |
| **2026-09-12** | **없는 셰이더 연출 처리.** 질문: 피격 흰 플래시(`BRB/SpriteFlash`)·시야 밖 어둠(`BRB/VisionDarkness`)·파손 오버레이(`BRB/DamageOverlay`) 셰이더가 없어 조용히 꺼져 있다 — 플래시·어둠 복구, 파손 제거 / 셋 다 복구 / 셋 다 정리만. **결정: 플래시·어둠 복구, 파손 제거**(사용자 선택). 플래시 = 3D 모델 재질을 잠깐 하얗게, 어둠 = URP 기본 반투명 셰이더. 파손 오버레이는 [destructible.md](destructible.md). |
