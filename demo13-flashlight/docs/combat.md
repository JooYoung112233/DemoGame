...
| **그로기** | 적만. 근접·총 모두 쌓이고 가득 차면 기절. 값은 StatDB(밴딧 80·중장 160) |
...
### 그로기 (적)
- 숨겨진 게이지 — **값은 StatDB가 진실**(2026-09-11 확인·사용자 결정 "데이터 유지"): 일반 밴딧 최대 **80**·초당 **10** 감소·기절 **2.5초** / 중장 **160**·**8**·**2.0초**
- 쌓는 양: 플레이어 약공 **5** · 강공 **25** · 풀차지 **45**(StatDB `playerStat`) · 권총 한 발 **14**(`WeaponData.groggy`)
- 강공격이 주요 축적 수단 — ⚠️ 약공만으로는 감쇠(10/s)를 못 넘는다(아래 2026-07-11 진단 참고)
...
### 배그식 사망 루팅 (2026-09-11 기획 추가 · 구현 대기)
...
> ⚠️ **2026-09-11 확인: 위 "신" 값은 StatDB에 한 번도 반영되지 않았다** — 데이터는 지금도 "구" 열(5·25·45 / 80·10·2.5 / 160·8·2.0)이다. 사용자 결정: **데이터 유지**, 문서를 데이터에 맞춘다(§그로기 (적)). 근접이 주력이 됐으니 플레이하며 다시 조정.
...
그로기 14 → 밴딧(maxGroggy **80**, 2026-09-11 데이터 기준) 기준 **6발 안팎**(감쇠 10/s 때문에 빠르게 연사해야 쌓인다). [→ balance.md](balance.md)
