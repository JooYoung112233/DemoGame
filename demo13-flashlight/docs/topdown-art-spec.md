...
| 2026-06-10 | 60° 바닥(80°)과 위화감 → 프랍 70°로 상향 | 60° 프랍은 너무 정면이라 80° 바닥과 20° 차 = **사물이 뒤로 누워 떠 보임**(사용자 지적). 결정(AskUserQuestion): **프랍 = ~70°로 세움**(바닥에 근접, 차 10°·윗면 지배+정면 ≤1/4). 바닥/벽은 80° 유지(타일링), **캐릭터·NPC는 60° 유지**(가독성). 앵커 박스 + 배치1·2 = 70°로 재생성 필요. |
