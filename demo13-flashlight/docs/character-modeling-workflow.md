...
- 사용자 결정: **“걷기, 달리기, 한손, 양손 으로 영상 다시 줄테니 애니 1개씩 만들고 컨펌 받자”**, **“걷기 만일단 해볼까”**. 걷기 한 개를 먼저 제작하고, 승인 전에 달리기나 공격을 새로 만들지 않는다.
- 참고 파일: 사용자 Downloads의 `걷기_달리기도_다시만들어_주고_지금_만들어준_한손_양.mp4`. 앞부분 대기/걷기/달리기는 기존 Blender 결과이며, 새 걷기 참고 구간은 약 9.67~10.75초다. 짧고 촬영 방향이 변하는 영상이므로 보폭·주기는 조형과 접지에 맞춰 해석하며 정확한 모션 캡처 복제라고 주장하지 않는다.
...
- 수정 방향: 무릎 들림과 상하 진폭을 줄이고 다리를 뻗어 디딤, 뒤꿈치 접지→발끝 밀기, 골반/상체의 반대 회전과 팔 흔들림. 30fps 1~37f, 1.2초 루프, 기준 이동 속도 약 0.417m/s. 게임 속도 변경 결정이 아닌 걷기 클립의 제작 기준이다.
- 확인: 루프 경계 정점 오차 0, 공중에 양발이 뜨는 프레임 0, 실제 부츠 접지 높이/동일 접촉 정점의 이동 보정 후 미끄러짐 0.000001m 미만, 다리 도달 거리 때문에 골반을 강제로 낮춘 양 0. [자세 비교](../Assets/ChibiSurvivor/Player/WalkReview/WalkPoses.png). 승인 및 Unity 연결 전이다.
- 생성/미리보기: `tools/build_walk_review.py`, `tools/preview_walk_review.py`. 아래 이전 공격 제작 결과는 보존된 이력이며 새로 승인된 참고 모션으로 간주하지 않는다.
