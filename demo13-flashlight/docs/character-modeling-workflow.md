...
- 최신 원인 지적: 사용자 **“머리가 고정인게 문제인것같은데”**. 머리 움직임을 먼저 분리해 비교한다. 이전 자세 연구의 머리 수평 고정과 실제 Run v3의 작은 앞뒤 기울기를 구분한다.
- 머리 실험: Run v3를 기준으로 Head 회전 키만 변경한 `HeadMotionReview/CompactSurvivor_RunHeadReview.blend`. 몸통·팔·다리, Head 위치 키, 승인 걷기 및 다른 액션은 모두 그대로 유지했다. 정적 자세 연구를 연결한 새 달리기가 아니다.
- 방향: 몸통 회전을 0.075초 늦게 따라오고, 착지 후 고개가 약간 숙여졌다 돌아오는 반동을 추가한다. 앞뒤 회전 폭 약 1.6→11.2도. 사용자가 아직 강도/느낌을 확인하지 않았으므로 승인 기준으로 고정하지 않는다.
- 비교 영상: `HeadMotionReview/HeadMotionComparison.mp4`, 왼쪽 Run v3 기존 머리/오른쪽 Head만 수정. 동일 몸 동작·카메라·재생 속도. 루프 경계 정점 오차 0, Head 회전 외 모든 모션 키 동일 확인. Unity 연결 전이다.

- 최신 피드백: **“음 좀 많이다른데”**. 달리기 v3도 미승인이다. 완료된 유사 모션이라고 취급하지 않는다.
- 현재 작업: 완성 루프의 수치 조정을 반복하기 전에 원본의 착지/밀어낸 직후/공중 자세 세 개를 직접 배치한 정적 연구안을 만든다. 머리는 수평에 가깝게 유지하고 몸통 전경, 앞에 머무르는 팔 자세, 앞뒤로 벌어지는 다리를 개별 참고 프레임에 맞춰 검토한다.
- 산출물: `Assets/ChibiSurvivor/Player/RunReview/KeyPoseStudy/RunKeyPoseStudy.blend`, `ReferenceKeyPoses.png`. 이 파일의 `Run_ThreePoseStudy_NOT_ANIMATION`은 정적 자세 3개이며 완성된 달리기 클립이 아니다. 기존 Walk/Run 파일은 변경하지 않았다. 자세 확인 전 추가 루프 제작은 보류한다.
...
- 원본 기준: 사용자가 **“보폭도 다르고 다리 움직임과 팔 벌리는것도 다르고 상체도 약간 숙이잖아 영상은”**이라고 구체적으로 교정했다. 작은 발 높이/흔들림 조절만으로 충분하다고 판단하지 않는다.
- 2026-09-09 원본 재첨부 후 `ArtSource/MotionReferences/LocomotionAndCombat_2026-09-09.mp4`에 보존했다. 10.667~12.833초를 24fps 연속 프레임으로 확인했으며, 실제 달리기 참고 비교에는 10.875~12.750초를 사용한다. 영상 앞쪽 기존 Blender 동작을 새 참고로 혼동하지 않는다.
- 최신 검토본: [CompactSurvivor_RunReview_v3.blend](../Assets/ChibiSurvivor/Player/RunReview/CompactSurvivor_RunReview_v3.blend). 승인 걷기 기반으로 Run만 변경했고, 승인 Walk·기존 나머지 4개 액션 및 외형/가중치를 보존한다. Player 본체와 Unity 연결 파일은 교체하지 않았다.
- v3 방향: 앞뒤로 벌어지는 보폭, 앞다리의 착지 전 신전과 뒷발 회수, 상체 전경 약 20도 수준의 골격 자세를 먼저 맞춘다. 기존 벌어진 휴식 팔 자세에 회전을 더하는 방식을 벗어나 어깨-팔꿈치-손목 경로를 몸 옆의 앞뒤 평면으로 설정한다. 팔꿈치를 굽혀 손이 가슴과 골반 옆을 오가게 한다.
- 제작 기준: 30fps, 1~25f, 0.8초 루프. 접지 궤적 0.58m와 지지 비율 0.34, 참고 이동 속도 약 2.132m/s는 애니메이션 제작 수치이며 게임 속도 결정이 아니다. 사용자의 달리기 승인은 계속 대기한다.
- 확인: 루프 경계 정점 오차 0, 접지 높이 오차 0.000001m 미만, 동일 접촉 정점의 이동 보정 후 프레임당 미끄러짐 최대 약 0.000392m, 강제 골반 높이 보정 0. 한 주기 양발 공중 프레임 6개. 손과 옷·바지·파우치·가방 표면 교차 없음. 모션 캡처나 원본 타이밍의 완전 복제로 주장하지 않는다.
- [참고 영상과 실제 Blender 비교](../Assets/ChibiSurvivor/Player/RunReview/RunReferenceComparison_v3.mp4): 왼쪽 원본 실제 구간, 오른쪽 수정 v3. 각자의 정상 속도로 재생하며 같은 포즈 시점을 강제로 동기화하지 않는다. [수정안 단독 영상](../Assets/ChibiSurvivor/Player/RunReview/RunPreview_v3.mp4)에서 반복 동작을 확인할 수 있다.
- 변경 이력: v1은 “달리기는 조금 어색한데”, v2는 보폭/다리/팔/전경 차이로 미승인이다. 두 파일은 `ArtSource/CharacterArchive/2026-09-09/RunReview_v1`, `RunReview_v2`에 보존했다. v3는 재첨부한 원본 연속 프레임을 기준으로 큰 자세부터 다시 제작했다.
