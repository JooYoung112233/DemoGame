...
| 문서 | 내용 | 상태 |
|------|------|------|
| [`combat.md`](combat.md) | 전투 — **총과 근접(방망이·검 등) 둘 다 주력, 좀보이드식**(2026-09-11 §무기 구성 결정). 총격(조준원·반동·탄 스펙, §총기), 배그식 사망 루팅(§사망 루팅). 맨손 공격 폐기, **구르기는 모션 전까지 꺼 둠**. 근접·스태미너·그로기는 2D 시절 설계라 정리 중. ⚠️ 문서 첫머리는 아직 "소울라이크 근접" — 정리 2단계에서 재작성 | 개정 필요 |
