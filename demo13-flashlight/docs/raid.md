...
### 시체 회수(사망 루프) — 「시간 잔상」 〔2026-07-10 확정 · 구현 전〕

- **사망 시 소지품(세큐어 컨테이너 제외)이 사망 지점에 「시간 잔상」 컨테이너로 남는다.** (기존 "가방 전부 소멸" 규칙(2026-06-02)을 대체 — 손실이 아니라 현장에 응고.)
- **세계관 정합**: 시계가 시간을 회수해 몸은 거점으로 역행하지만, 소지품은 역행에서 제외돼 그 자리에 응고 — [gdd-core §5](gdd-core.md) 사망=시간역행 룰과 연결.
- **잔상은 최신 1개만** — 다시 죽으면 이전 잔상은 현상에 삼켜져 소멸. **유지 기한 없음**(다음 사망까지).
- **회수** = 잔상을 열어 꺼내기(컨테이너 상호작용).
- 구현 접점: **SaveManager 영속 필요**(병렬 핫파일 — 구현 시점 조율 필요). 구현 순서: A+B 통합 10종 중 **8번째**(dev-roadmap.md 2026-07-10).
