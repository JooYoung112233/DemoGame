...
## 2026-09-13 — 앞단 신규 유저 플레이 검증

- 요청: “시퀀스 검증, 앞단 플레이하면서”. 타이틀/빈 슬롯 새 게임 → 프롤로그 → 하이드아웃 장비 준비/왕복 → 마을 NPC/전당포 → 출전 지도/의뢰 → Zone1 첫 이동을 Unity MCP로 실행했다.
- **전체 스토리 시퀀스 미통과**: 전당포 첫 대면 S-002가 그레이박스 테스트용 비활성 상태. 암구호가 없어 회수꾼 S-003/S-004 조건이 열리지 않고, 새 게임 의뢰 게시판도 비어 있다. 기존 비활성 정책은 이번 검증에서 바꾸지 않았다.
- 발견한 프롤로그 독백 가림/암전 위 입구 표기, 시설창→인벤토리 중첩을 수정하고 새 게임으로 재검증했다. SaveManager의 최종 JSON 쓰기도 QA 억제를 준수하도록 보완했다. 관련 회귀 **10/10**, 콘솔 오류 0건. 전체 게임 통과 수치가 아니다.
- **남은 UX**: 창고 시설은 건설 화면이고 기본 보관함은 인벤토리에 있어 혼동됨. 초기 가방/보급 안내, 회색 지형 위주인 레이드 첫 시야/목표 안내가 부족하다.
- `QA_ENABLED`는 비활성 그대로. MCP 하네스의 GameInput 가상 입력 및 uGUI 실제 버튼 리스너를 사용했다. 타이틀/거래 버튼의 OS 클릭 검증이나 레이드 루팅·탈출·정산 완주는 포함하지 않는다.
- [전체 판정·재현·화면·오탐 구분](../../ArtWork/FrontSequence/README.md). 실제 저장 파일 해시 동일, 테스트 슬롯 미생성, 원래 정지/씬 구성 복원.
