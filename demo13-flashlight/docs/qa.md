...
`GameTuning.cs`의 `[Header]`/`[Tooltip]`/`[Range]`/기본값을 파싱해 `GameTuning.asset`의
현재 값과 짝지어 보여준다. 저장은 **해당 줄의 값만 치환**하므로 Unity 서식·메타가 보존된다
(에셋에 아직 없는 필드는 마지막 키 뒤에 추가). 기본값과 다른 항목은 색으로 구분.
