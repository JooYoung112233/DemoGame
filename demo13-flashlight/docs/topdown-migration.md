...
- 조명은 URP **2D Light**. ⚠️ 손전등 주광원 안은 **2026-06-02 후속 폐기 → 좀보이드식 시야(FOV)로 전환** (`combat.md`/`rendering.md` 참조)

## 핵심 결정
| 날짜 | 질문 | 결정 |
|------|------|------|
| 2026-06-02 | 아이소를 계속? 탑다운으로? | **탑다운 2D 전환.** 기존 아이소 렌더링 제거, 게임 로직 유지 |
| 2026-06-02 | 카메라/아트 | 카메라 2D Orthographic, 아트에 ~80° 틸트 베이크, 모든 오브젝트 회전 (0,0,0) |
| 2026-06-02 | 조명/가시성 | URP 2D Light. ~~손전등 주광원~~ → **시야 FOV로 전환**(후속 결정, 손전등 폐기) |
...
| | OcclusionOutline·WallOcclusionOutline | 시야 FOV(콘+하이브리드 어둠) |
| | 손전등(FlashlightController·Light2D) ※폐기 | TilemapCollider2D |
...
- [x] ~~**2. 2D 손전등**~~ — (구현됨) 플레이어 자식 Light2D, 마우스 방향, 글로벌 앰비언트. **⚠️ 2026-06-02 폐기 결정 → 시야 FOV로 대체. `FlashlightController`/`FlashlightBeam`/손전등 Light2D 제거 대상.**
...
- [x] **5. 게임 로직 재연결** — 상호작용/문/루팅을 Collider2D로. `IInteractable.Interact(GameObject)` 시그니처 전환. 의존 파일 `TopDownPlayer` 재연결 **완료**(`TODO(TopDownPlayer)` 잔재 0).
...
- [ ] **8. 시야(FOV) 시스템** — 손전등 제거 후 좀보이드식 시야 콘 + 하이브리드 어둠 + 적 가시성 판정. (2026-06-02 결정, 미착수)
- [ ] **9. 타격감 연출** — 강공 히트스탑 + 적 셰이더 플래시 + 플레이어 피격 화면연출 + 카메라 셰이크/줌. (2026-06-02 설계, 미착수. `combat.md`)

> **남은 일**: 씬 콘텐츠 채우기(맵 페인팅) · **시야 FOV 시스템 신규 + 손전등 코드 완전 제거** · **타격감 연출 구현**(셰이더 플래시·히트스탑·피격연출·카메라).
...
| 2026-06-02 | **계획 정합화(후속 결정 반영).** 손전등 "유지"→**폐기(시야 FOV 전환)**로 갱신: 왜/핵심결정/유지·제거·신규 표·step2 모두 supersede 표기. step5(게임로직 재연결)는 `TODO(TopDownPlayer)` 잔재 0 확인 → **완료([x])**. **step8(시야 FOV)·step9(타격감 연출) 신규 추가**. (상세 결정은 `combat.md`/`rendering.md`.) |
| 2026-06-02 | **TopDownPlayer 프리팹 생성 + 잔재 정리.** 탑다운 전환 후 사라진 플레이어 프리팹을 복구 — `Resources/TopDownPlayer.prefab` 신규. 깨진 구 `Player.prefab`(삭제된 스크립트 3개 missing 참조, 프로젝트 참조 0)을 대체·삭제. 재사용 빌더 `TopDownPlayerBuilder.cs`(메뉴 `Tools▸TopDown 2D▸Build Player Prefab`)로 코드 조립(Unity 배치모드 실행으로 생성) → 컴포넌트 10종(Rigidbody2D·CircleCollider2D·TopDownPlayer·Health·PlayerInventory·PlayerMedicalSystem·InteractionSystem·CombatFeedback·MedicalHUD·FlashlightController) + 자식 3개(PlayerSprite·Hurtbox(trigger)·FlashlightPivot(Light2D)). **Player 태그**(EnemyController가 `FindGameObjectWithTag`로 탐지)·Player 레이어·enemyMask=Enemy(512)·spotLight 와이어링 완료, missing 0. 추가 정리: 고아 `FogOfWar.meta`, 공백 파일명 `RaidResultUI .prefab` 삭제. `story-script.md` stale 제약(쿼터뷰/빌보드)→탑다운 2D 교정. |
