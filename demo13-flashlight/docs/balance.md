...
- `Resources/Data/GameTuning.asset` → `interactionDistance`: 상호작용 상한 거리 **1.8m**, Control Panel 범위 0.5~4m, 런타임 즉시 적용. 개별 오브젝트의 더 짧은 범위는 유지한다.
- `Resources/Data/StarterArmory.json`: 안전구역 1회 지급 구성. 권총(15발 탄창 포함), 방망이, 15발 예비 탄창, 9mm 탄약 30발. 무기 전투 수치는 기존 WeaponData를 사용한다.
