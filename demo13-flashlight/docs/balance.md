...
| **도로 장애물·막힌 통로** (장애물 밀도·최소 통행폭, 잔해 치우기/강제돌파 시간·소음 반경) | **`GameTuning`** (`roadObstacleDensity`/`roadMinPassWidth`/`barricadeClearSeconds`/`barricadeBreachSeconds`/`barricadeNoiseRadius`) | **Control Panel** — 밀도·통행폭은 `빌드 ▸ 지역1` 재실행 시 반영, 시간·소음은 런타임 즉시. `barricadeNoiseRadius`=0이면 지름길이 공짜가 된다 (→ [level-scrapmarket.md 2026-07-11](level-scrapmarket.md)) |
