...
## 2026-09-13 — 앞단 시퀀스 플레이 QA

- 새 게임→프롤로그→하이드아웃 장비/왕복→전당포 대화/거래→출전→Zone1 첫 이동을 MCP로 검증. **스토리 흐름은 미통과**: 전당포 S-002 분기가 테스트용으로 비활성되어 암구호/회수꾼/첫 임무로 이어지지 않는다. 새 슬롯 게시판은 실제로 비어 있음.
- 프롤로그 독백을 암전 위에 표시하고 연출 중 입구 마커를 숨겼다. 하이드아웃 시설창 뒤에서 인벤토리가 열리는 중첩을 수정. SaveManager.WriteJson에도 SuppressWrites 적용. 새 게임 재검증 10/10, 오류 0건.
- 다음 우선순위: 기존 테스트용 스토리 비활성을 해제할 범위 정리→S-002/S-003/S-004/MQ-001 연결→첫 가방/장비/창고 안내→레이드 첫 목표·루팅·탈출 시퀀스. 이번에 해당 스토리 정책/보급 수치/맵 아트는 변경하지 않음.
- 실제 세이브 해시 동일, 테스트 슬롯 미생성. Unity는 원래 Play 정지, Systems 활성+Safehouse additive, dirty=false로 복원. [qa.md](qa.md), `ArtWork/FrontSequence/README.md`에 증거와 하네스 오탐 구분.

## 2026-09-13 — 공격 중 머리 높이에 남던 착용등

- 사용자: “휘두를 때 뒤통수에 라이트가 박혀 있는 게 보인다.” 실제 광원 PlayerLamp3D가 루트 Y=1.15m에 고정된 것을 확인했다. Game View 기즈모는 꺼져 있었다. 전구 아이콘/실제 빛 구분 질문은 회신 대기 상태로 실제 광원 배치부터 수정했다.
- WornLamp가 LateUpdate에서 animated Chest + 회전 기준 (.12,-.04,.22)m를 따라간다. FBX 뼈 스케일 100은 제외. 빔 방향·밝기·등급 정책은 그대로이며 주변광 배치 전에 본체 위치를 갱신한다.
- Unity MCP 11/11 통과, 세 공격 923회 샘플에서 머리/모자 바운드 관통 0회, 가슴 부착 오차 0. 자체 게임 화면 검수 및 오류 0건. `ArtWork/LampClearance` 참고. 테스트 장비/위치/낮밤 적용을 복원했고 디스크 저장을 억제했다.
