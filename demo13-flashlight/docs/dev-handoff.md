...
- **PR 상태**: #16~#32는 모두 `main`에 병합됐다(2026-09-12 확인). 새 작업은 브랜치를 따서 PR로 올린다(`main` 직접 push 금지).
- **진행 중 = 시스템 정리 6단계** ([dev-roadmap.md §시스템 정리](dev-roadmap.md)):
  1 입구 문서 ✅ → 2 전투·총기 ✅(총·근접 둘 다 주력, 구르기 꺼 둠, 퀵슬롯 전환, 총 수치 WeaponData 통합) → 3 루팅·경제 ✅(재화 역할 분리, 루팅 정리) → **4 거점**(다른 세션 작업 종료 후) → 5 조명 잔재·레이드/실내 흐름 → 6 폴더·거대 파일.
