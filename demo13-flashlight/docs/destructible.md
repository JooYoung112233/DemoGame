...
> ✅ **2026-09-12 결정: 파손 오버레이 제거.** 질문(없는 셰이더 연출 3종 처리 — 플래시·어둠 복구·파손 제거 / 셋 다 복구 / 셋 다 정리만) → 사용자: **플래시·어둠 복구, 파손 제거**. `Breakable`의 오버레이 코드와 같은 셰이더만 쓰던 `Weathered`를 지운다. 단계별 부서짐·파괴 로직은 유지한다.
>
> (결정 전 상태) **오버레이는 동작하지 않았다.** 셰이더 `BRB/DamageOverlay`가 프로젝트에 없다(2026-09-10 셰이더 정리 — 남은 게임 셰이더는 `BRB/GameLit` 하나). `Breakable`·`Weathered`는 경고 로그를 남기고 오버레이 없이 진행하며, 오버레이 구조(SpriteRenderer 자식)도 2D 시절 방식이라 3D 오브젝트에는 붙지 않는다. 아래는 2D 시절 설계 기록이고, 3D 파손 연출은 아직 정하지 않았다.
