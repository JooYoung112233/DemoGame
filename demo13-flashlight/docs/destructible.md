...
> ⚠️ **2026-09-12 현재 오버레이는 동작하지 않는다.** 셰이더 `BRB/DamageOverlay`가 프로젝트에 없다(2026-09-10 셰이더 정리 — 남은 게임 셰이더는 `BRB/GameLit` 하나). `Breakable`·`Weathered`는 경고 로그를 남기고 오버레이 없이 진행하며, 오버레이 구조(SpriteRenderer 자식)도 2D 시절 방식이라 3D 오브젝트에는 붙지 않는다. 아래는 2D 시절 설계 기록이고, 3D 파손 연출은 아직 정하지 않았다.
