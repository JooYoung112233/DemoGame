const string folder="Assets/ChibiSurvivor/Player/DarkSurvivor/SurfaceReview";
...
try{
 var instance=(UnityEngine.GameObject)UnityEditor.PrefabUtility.InstantiatePrefab(UnityEditor.AssetDatabase.LoadAssetAtPath<UnityEngine.GameObject>("Assets/ChibiSurvivor/Player/DarkSurvivor/DarkSurvivor.prefab"),preview);
 var animator=instance.GetComponentInChildren<UnityEngine.Animator>();animator.enabled=false;
 var clip=UnityEditor.AssetDatabase.LoadAllAssetsAtPath("Assets/ChibiSurvivor/Player/DarkSurvivor/DarkSurvivor.fbx").OfType<UnityEngine.AnimationClip>().Single(c=>c.name=="Idle");clip.SampleAnimation(instance,0);
 ...
 foreach(var mode in new[]{"Neutral","Dark"}){
  ...
  camera.Render();UnityEngine.RenderTexture.active=rt;pixels.ReadPixels(new UnityEngine.Rect(0,0,900,1000),0,0);pixels.Apply();System.IO.File.WriteAllBytes(folder+"/Unity_"+mode+".png",pixels.EncodeToPNG());
 }
 return new{images=new[]{folder+"/Unity_Neutral.png",folder+"/Unity_Dark.png"},playStatePreserved=playing==UnityEditor.EditorApplication.isPlaying,activeScenePreserved=sceneBefore==UnityEngine.SceneManagement.SceneManager.GetActiveScene()};
}finally{UnityEngine.RenderTexture.active=previousRT;if(pixels!=null)UnityEngine.Object.DestroyImmediate(pixels);if(rt!=null){rt.Release();UnityEngine.Object.DestroyImmediate(rt);}UnityEditor.SceneManagement.EditorSceneManager.ClosePreviewScene(preview);}
