...
foreach(var mode in new[]{"StowedFront","StowedBack","Aim","Walk","Shoot"}){
 ...
 try{
  for(int i=0;i<2;i++){
   ...
   if(!mode.StartsWith("Stowed")){
    UnityEditor.AssetDatabase.LoadAssetAtPath<AnimationClip>(root+"/Animations/"+kind+"_"+mode+".anim").SampleAnimation(go,mode=="Walk"?.25f:mode=="Shoot"?.12f:0);
    gun.SetParent(bones.Single(t=>t.name=="FirearmGrip"),false);gun.localPosition=new Vector3(0,.035f,0);gun.localRotation=Quaternion.identity;
   }
  ...
  utility.BeginStaticPreview(new Rect(0,0,1200,800));utility.Render(true);var png=utility.EndStaticPreview();System.IO.File.WriteAllBytes(output+"/"+mode+".png",png.EncodeToPNG());UnityEngine.Object.DestroyImmediate(png);rows.Add(mode);
