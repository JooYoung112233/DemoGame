...
try{
 var instance=UnityEngine.Object.Instantiate(UnityEditor.AssetDatabase.LoadAssetAtPath<GameObject>(folder+"/SimpleHero.prefab"));
 var clip=UnityEditor.AssetDatabase.LoadAssetAtPath<AnimationClip>(folder+"/BatSwing.anim");clip.SampleAnimation(instance,0);
 foreach(var r in instance.GetComponentsInChildren<Renderer>(true)){if(r.name=="Hero_Bat")r.enabled=true;if(r.name=="Hero_SwordProxy")UnityEngine.Object.DestroyImmediate(r.gameObject);}
 ...
 var path="D:/Demo/ArtWork/SimpleHeroMelee/Unity_BatHold.png";System.IO.File.WriteAllBytes(path,texture.EncodeToPNG());UnityEngine.Object.DestroyImmediate(texture);return path;
