...
try{
 ...
 foreach(float speed in new[]{0f,1f,2f}){
  animator.Play("PatrolLocomotion",0,0);animator.SetFloat("Speed",speed);animator.Update(0);
  for(int i=0;i<180;i++){
   animator.Update(1f/120);skin.BakeMesh(baked);var vertices=baked.vertices;var matrix=Matrix4x4.TRS(skin.transform.position,skin.transform.rotation,Vector3.one);
 ...
 var report=new{passed=true,maxPalmError=maxPalm,minTipHeight=minTip,minFreeHandDistance=minFreeHand,checks};var dir="D:/Demo/ArtWork/SimpleBandit/PatrolCarry";System.IO.Directory.CreateDirectory(dir);System.IO.File.WriteAllText(dir+"/Checks.json",Newtonsoft.Json.JsonConvert.SerializeObject(report,Newtonsoft.Json.Formatting.Indented));return report;
