const string folder="Assets/ChibiSurvivor/Bandit/SimpleBandit";const string output="D:/Demo/ArtWork/SimpleBandit/PatrolCarry";
...
tick=()=>{
 try{
  float t=frame/30f;
  float speed=t<1.5f?0:t<1.8f?Mathf.SmoothStep(0,1,(t-1.5f)/.3f):t<4.3f?1:t<4.6f?Mathf.SmoothStep(1,0,(t-4.3f)/.3f):t<7.5f?0:t<7.8f?Mathf.SmoothStep(0,1,(t-7.5f)/.3f):1;
  animator.SetFloat("Speed",speed);if(frame==90)animator.CrossFadeInFixedTime("Locomotion",.28f);if(frame==157)animator.SetTrigger("Attack");if(frame==225)animator.CrossFadeInFixedTime("PatrolLocomotion",.28f);animator.Update(1f/30);
  skin.BakeMesh(baked);
