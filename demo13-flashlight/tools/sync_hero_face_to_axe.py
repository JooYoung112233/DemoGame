...
ROOT=Path(__file__).resolve().parents[1];CACHE=ROOT/'Library/CodexBlender/HeroFaceTexture'
...
for path,fbx in targets:
 backup=CACHE/(path.stem+'_BeforeFace.blend')
 ...
 def invariant():
  return hashlib.sha256(repr(([(o.name,[tuple(v.co) for v in o.data.vertices], [[(g.group,g.weight) for g in v.groups] for v in o.data.vertices]) for o in bpy.context.scene.objects if o.type=='MESH' and o.name not in ('Hero_FaceDetails','Hero_Nose')],[(b.name,tuple(b.head_local),tuple(b.tail_local)) for b in rig.data.bones],[(a.name,[(f.data_path,f.array_index,[(tuple(k.co),tuple(k.handle_left),tuple(k.handle_right)) for k in f.keyframe_points]) for f in a.fcurves]) for a in bpy.data.actions])).encode()).hexdigest()
...
(CACHE/'AxeFaceSync.json').write_text(json.dumps(reports,indent=2));print('AXE_FACE_SYNC_READY',flush=True)
