...
def fit(pose):
 ...
 for _ in range(3):
  clearaxis=(axis+chestforward*max(0,.35-axis.dot(chestforward))).normalized()
  if clearaxis.z>.15:clearaxis=(clearaxis+forward*max(0,clearaxis.z*1.8-clearaxis.dot(forward))).normalized()
 ...
 # Keep the swing outside the larger head and chest of the compact body.
 relative=chestq.inverted()@(anchor-rig.pose.bones['Chest'].head)
 relative.x=max(-.17,min(.17,relative.x));relative.y=max(-.34,min(-.25,relative.y));relative.z=max(-.04,min(.025,relative.z))
...
proxy=[]
for label,pos,size,mat in [('Grip',(0,-.07,0),(.043,.255,.043),leather),('Guard',(0,.095,0),(.12,.025,.04),steel),('Blade',(0,.462,0),(.065,.70,.022),steel)]:
...
(OUT/'Stage5SlashCheck.json').write_text(json.dumps(report,indent=2));print('SLASH_CHECK',report,flush=True)
assert max(suberrors)<.008,('Interpolated grip detached',max(suberrors))
assert not hits,('Weapon intersects character',hits[:8])
...
sc.render.resolution_x=640;sc.render.resolution_y=720;sc.render.resolution_percentage=100;sc.eevee.taa_render_samples=24;sc.render.image_settings.file_format='PNG'
for f in [1,10,19,28]:
