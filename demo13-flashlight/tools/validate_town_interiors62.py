...
for name,b in m['buildings'].items():
 ...
 for p in m['layouts'][name]:
  ...
  assert abs(x)+ex<=b['width']/2-.16+.04,(name,p['asset'],'side wall overlap')
  assert abs(z)+ez<=b['depth']/2-.16+.04,(name,p['asset'],'front/rear wall overlap')
  rects.append((x-ex-radius,x+ex+radius,z-ez-radius,z+ez+radius))
 def free(x,z):
  return abs(x)<b['width']/2-.18-radius and abs(z)<b['depth']/2-.18-radius and not any(x1<x<x2 and z1<z<z2 for x1,x2,z1,z2 in rects)
 nx=math.floor((b['width']-1)/step);nz=math.floor((b['depth']-1)/step)
 def pos(i,j):return ((i-nx/2)*step,(j-nz/2)*step)
