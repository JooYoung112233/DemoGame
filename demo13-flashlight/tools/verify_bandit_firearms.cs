...
try{foreach(var kind in new[]{"Pistol","Rifle"}){
 ...
 float maxStep=0,maxStepTime=0,maxRight=0,maxLeft=0,maxStockDistance=0,minWeaponRight=100;Vector3 previous=Vector3.zero;int bad=0;
 for(int i=0;i<=840;i++){
  ...
  if(actor.IsAiming){maxRight=Mathf.Max(maxRight,Vector3.Distance(right.position,weapon.TransformPoint(new Vector3(0,-.035f,0))));if(kind=="Rifle"){
   maxLeft=Mathf.Max(maxLeft,Vector3.Distance(left.position,weapon.TransformPoint(new Vector3(0,-.04f,.155f))));
   minWeaponRight=Mathf.Min(minWeaponRight,go.transform.InverseTransformPoint(weapon.position).x);
   maxStockDistance=Mathf.Max(maxStockDistance,Vector3.Distance(weapon.TransformPoint(new Vector3(0,.06f,-.21f)),bones.Single(t=>t.name=="UpperArm.R").position));
  }}
 }
 ...
 if(kind=="Rifle"&&(minWeaponRight<.12f||maxStockDistance>.14f))throw new System.Exception("Rifle must stay beside the right shoulder");
 ...
 rows.Add(new{kind,maxWeaponFrameStep=maxStep,maxStepTime,maxRightGripError=maxRight,maxLeftGripError=maxLeft,maxStockDistance,minWeaponRight=kind=="Rifle"?minWeaponRight:0,invalidTransforms=bad,stowedParent,armedParent,weaponWorldScale=scale});
