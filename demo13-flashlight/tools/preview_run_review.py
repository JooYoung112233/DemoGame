...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'Assets/ChibiSurvivor/Player/RunReview'
OLD=ROOT/'ArtSource/CharacterArchive/2026-09-09/RunReview_v2/CompactSurvivor_RunReview.blend'
CACHE=ROOT/'Library/CodexBlender/RunReview_v3';CACHE.mkdir(exist_ok=True)
bpy.ops.wm.open_mainfile(filepath=str(OUT/'CompactSurvivor_RunReview_v3.blend'))
bpy.context.preferences.filepaths.save_version=0
scene=bpy.context.scene;rig=bpy.data.objects['CompactSurvivor_Rig'];new=bpy.data.actions['Run']
meshes=[o for o in scene.objects if o.type=='MESH'];stats=json.loads((OUT/'RunCheck.json').read_text())
...
if '--poses' in sys.argv:
    ...
    label('직전 달리기',-.6,3.59);label('수정한 달리기',.6,3.59)
    ...
    scene.cycles.samples=16;scene.render.image_settings.file_format='PNG';scene.render.filepath=str(OUT/'RunPoses_v3.png')
    ...
    scene.camera.location=(-6.7,-4.5,2.3);target=Vector((0,0,.79));scene.camera.rotation_euler=(target-scene.camera.location).to_track_quat('-Z','Y').to_euler();scene.camera.data.ortho_scale=1.98
    ...
    movie_settings()
    views=[('Revised',new,stats['nominal_forward_speed_mps'])] if '--render-only' in sys.argv else [('Previous',old,.37/(.4*24/30)),('Revised',new,stats['nominal_forward_speed_mps'])]
    for title,action,speed in views:
        ...
        if title=='Revised':bpy.ops.wm.save_as_mainfile(filepath=str(OUT/'RunPreview_v3.blend'))
    ...
    for col,(title,body) in enumerate([('Previous','직전 달리기'),('Revised','수정한 달리기')]):
        for rep in range(2):
    ...
    scene.frame_start=1;scene.frame_end=240;path=OUT/'RunComparison_v3.mp4';scene.render.filepath=str(path)
    ...
    (OUT/'BodyPreviewCheck.json').write_text(json.dumps({'frames':240,'fps':30,'seconds':8.0,
      'left':'Run v2','right':'Run v3','same_camera':True,'each_clip_natural_cadence':True,
