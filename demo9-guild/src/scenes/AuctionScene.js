...
class AuctionScene extends Phaser.Scene {
    ...
    _drawBidCard(item, x, y, w, h) {
        ...
        if (item.stats) {
            ...
            this._add(this.add.text(x + 16, y + 60, statStr, {
                fontSize: '11px', fontFamily: 'monospace', color: '#aaaaa0', wordWrap: { width: w - 32 }
            }));
        }
        // 패시브 효과 (rare+)
        if (item.passiveData) {
            this._add(this.add.text(x + 16, y + 78, `◆ ${item.passiveData.name}: ${item.passiveData.desc}`, {
                fontSize: '10px', fontFamily: 'monospace', color: '#66ccff', wordWrap: { width: w - 32 }
            }));
        }
        // 세트 표시 (epic+)
        if (item.setData) {
            this._add(this.add.text(x + 16, y + 94, `🔗 세트: ${item.setData.name}`, {
                fontSize: '10px', fontFamily: 'monospace', color: '#88ff88'
            }));
        }
        if (item.desc) {
            this._add(this.add.text(x + 16, y + 110, item.desc, {
    ...
    _drawRevealCard(item, x, y, w, h) {
        ...
        if (item.passiveData) {
            this._add(this.add.text(x + 16, y + 52, `◆ ${item.passiveData.name}`, {
                fontSize: '10px', fontFamily: 'monospace', color: '#66ccff'
            }));
        }
    ...
    _drawSellingItemCard(item, x, y, w, h) {
        ...
        this._add(this.add.text(x + 38, y + 30, `[${rarity.name}]${item.passiveData ? ' ◆' + item.passiveData.name : ''}`, {
            fontSize: '9px', fontFamily: 'monospace', color: '#888899'
        }));
        this._add(this.add.text(x + 10, y + 48, `시세 ${item.marketPrice}G  /  매입 ${item._result.paid}G`, {
            fontSize: '10px', fontFamily: 'monospace', color: '#aaaaaa'
        }));
        if (item.setData) {
            this._add(this.add.text(x + 10, y + 63, `🔗 ${item.setData.name}`, {
