class TempleScene extends Phaser.Scene {
    ...
    _drawHealSection(gs, x, y, w) {
        const T = (typeof UI_THEME !== 'undefined') ? UI_THEME : {};
        this._add(UIPanel.create(this, x, y, w, 560, { title: '전체 치유', ornament: true }));
        ...
        this._add(this.add.text(x + w / 2, y + 30, `부상 용병: ${injured.length}명`, {
            fontSize: '12px', fontFamily: T.fontFamily || 'monospace', color: injured.length > 0 ? '#ff8844' : '#44ff88'
        }).setOrigin(0.5));
        ...
        gs.roster.forEach(merc => {
            ...
            this._add(this.add.text(x + 20, cy + 6, `${base.icon} ${merc.name}`, {
                fontSize: '12px', fontFamily: T.fontFamily || 'monospace', color: T.textPrimary || '#e8d8c0', fontStyle: 'bold'
            }));
            ...
            if (isHurt) {
                ...
                this._add(this.add.text(x + w - 60, cy + 14, '만렙', {
                    fontSize: '10px', fontFamily: T.fontFamily || 'monospace', color: '#44ff88'
                }).setOrigin(0.5));
            }
    ...
    _drawRevivalSection(gs, x, y, w) {
        const T = (typeof UI_THEME !== 'undefined') ? UI_THEME : {};
        this._add(UIPanel.create(this, x, y, w, 560, { title: '부활 / 부활권', ornament: true }));

        // === 사망자 부활 ===
        const fallen = gs.fallenMercs || [];
        if (fallen.length > 0) {
            this._add(this.add.text(x + w / 2, y + 22, `💀 사망 용병 ${fallen.length}명`, {
                fontSize: '11px', fontFamily: T.fontFamily || 'monospace', color: '#ff6666', fontStyle: 'bold'
            }).setOrigin(0.5));
            ...
            fallen.slice(0, showCount).forEach(merc => {
                ...
                this._add(this.add.text(x + 20, fy + 4, `${fBase.icon} ${merc.name}`, {
                    fontSize: '11px', fontFamily: T.fontFamily || 'monospace', color: fRarity.textColor, fontStyle: 'bold'
                }));
        ...
        div.lineStyle(1, T.divider || 0x5a4a2a, 0.5);
        div.lineBetween(x + 20, divY, x + w - 20, divY);

        this._add(this.add.text(x + w / 2, divY + 12, '── 부활권 (사전 예방) ──', {
            fontSize: '10px', fontFamily: T.fontFamily || 'monospace', color: T.textPrimary || '#e8d8c0', fontStyle: 'bold'
        }).setOrigin(0.5));
        ...
        gs.roster.forEach(merc => {
            ...
            const bg = this._add(this.add.graphics());
            bg.fillStyle(hasRevival ? 0x2a2a1a : (T.cardFill || 0x231e14), 1);
            bg.fillRoundedRect(x + 10, cy, w - 20, 44, 3);
            bg.lineStyle(1, hasRevival ? 0x886644 : (T.panelStroke || 0x5a4a2a), 0.3);
            ...
            this._add(this.add.text(x + 20, cy + 24, `Lv.${merc.level} ${RARITY_DATA[merc.rarity]?.name || merc.rarity}`, {
                fontSize: '9px', fontFamily: T.fontFamily || 'monospace', color: rarityColor
            }));

            if (hasRevival) {
                this._add(this.add.text(x + w - 60, cy + 14, '✨ 부활권', {
                    fontSize: '11px', fontFamily: T.fontFamily || 'monospace', color: T.textGold || '#ffcc44', fontStyle: 'bold'
                }).setOrigin(0.5));
            } else {
                this._add(UIButton.create(this, x + w - 60, cy + 22, 80, 24, `${revivalCost}G`, {
                    ...
                    textColor: canBuy ? (T.textGold || '#ffcc44') : '#555555',
                    fontSize: 10,
                    onClick: () => {
                        if (!canBuy) return;
                        ...
                        SaveManager.save(gs);
                        UIToast.show(this, `${merc.name}에게 부활권 장착!`, { color: T.textGold || '#ffcc44' });
                        this.goldText.setText(`${gs.gold}G`);
                        this._drawContent();
                    }
                }));
    ...
    _drawBlessSection(gs, x, y, w) {
        const T = (typeof UI_THEME !== 'undefined') ? UI_THEME : {};
        this._add(UIPanel.create(this, x, y, w, 560, { title: '출발 축복 (다음 전투 적용)', ornament: true }));
        ...
        blessings.forEach(bless => {
            ...
            const bg = this._add(this.add.graphics());
            bg.fillStyle(active ? 0x1a2a3a : (T.cardFill || 0x231e14), 1);
            bg.fillRoundedRect(x + 10, cy, w - 20, 60, 3);
            bg.lineStyle(1, active ? 0x4488ff : (T.panelStroke || 0x5a4a2a), 0.4);
            ...
            this._add(this.add.text(x + 55, cy + 8, bless.name, {
                fontSize: '13px', fontFamily: T.fontFamily || 'monospace', color: bless.color, fontStyle: 'bold'
            }));
            ...
            this._add(this.add.text(x + 55, cy + 42, `${bless.cost}G`, {
                fontSize: '10px', fontFamily: T.fontFamily || 'monospace', color: T.textGold || '#ffcc44'
            }));

            if (active) {
                this._add(this.add.text(x + w - 60, cy + 22, '✓ 활성', {
                    fontSize: '11px', fontFamily: T.fontFamily || 'monospace', color: '#44ff88'
                }).setOrigin(0.5));
            } else {
