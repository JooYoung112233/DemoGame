...
## 6. 99단계 밸런스 수정 계획 (2026-05-18 확정, 2026-05-19 구현 완료)

### 6.0 결정 사항

| 항목 | 결정 | 근거 |
|------|------|------|
| 용병 레벨캡 | **30** | balance.js 예정값 채택. ZL30까지 레벨 성장 커버, 이후 장비 의존 |
| 구역별 난이도 | **해금레벨 기준 자동 보정** | 구역 해금 시점의 길드레벨 기대 파워에 맞춰 ZL1 스케일 자동 산출 |
| 고레벨(50+) | **장비 필수 벽** | ZL30+ 레벨만으론 부족, 에픽/전설 장비 + 강화가 필수 |

### 6.1 문제 진단

**현재 파워 갭:**
- 용병 Lv10 전설 전사: HP 982, ATK 70, DEF 67
- ZL10 적 scaleMult 1.42x → Pitlord HP 2,130, ATK 64 (대응 가능)
- ZL30 적 scaleMult 2.62x → Pitlord HP 3,930, ATK 118 (불가능 — 성장 수단 없음)
- ZL99 적 scaleMult 6.76x → Pitlord HP 10,140, ATK 304 (절대 불가)

**구역별 ZL1 불균형:**
- BP: 0.75x 감소 → Runner HP 52, ATK 7 (쉬움)
- Cargo: 1.0x 무감소 → Shielder HP 300, DEF 20, Bomber ATK 30 (너무 어려움)
- Blackout: 1.0x → Bone Golem HP 400 (너무 어려움)

### 6.2 용병 성장 곡선 (Lv1→30)

**XP 커브**: `50 × lv^1.5` (balance.js 예정 공식 적용)

| Lv | 필요 XP | 누적 XP | 전사 HP | 전사 ATK | 전사 DEF |
|---|---|---|---|---|---|
| 1 | - | 0 | 250 | 20 | 18 |
| 5 | 559 | 1,330 | 370 | 28 | 26 |
| 10 | 1,581 | 6,907 | 520 | 38 | 36 |
| 15 | 2,905 | 17,716 | 670 | 48 | 46 |
| 20 | 4,472 | 34,708 | 820 | 58 | 56 |
| 25 | 6,250 | 58,671 | 970 | 68 | 66 |
| 30 | 8,216 | 90,000~ | 1,120 | 78 | 76 |

**전설 Lv30**: HP 1,680, ATK 117, DEF 114

### 6.3 적 스케일링 공식 (통일)

**기존 문제**: ManualBattle, enemies.js, CargoBattleScene이 각각 다른 공식 사용.

**통일 공식 (balance.js에 추가):**
```
ZONE_SCALE = {
  // 해금레벨 기준 보정 — 해당 구역 진입 시점의 기대 파워에 맞춤
  zoneBaseMult: { bloodpit: 0.65, cargo: 0.70, blackout: 0.75 },
  // 레벨당 스케일 증가율
  scalePerLevel: 0.06,
  // 최종: baseMult + (zoneLv - 1) * scalePerLevel
}
```

| 구역 | ZL1 | ZL5 | ZL10 | ZL20 | ZL30 | ZL50 | ZL99 |
|------|-----|-----|------|------|------|------|------|
| BP | 0.65 | 0.89 | 1.19 | 1.79 | 2.39 | 3.59 | 6.53 |
| Cargo | 0.70 | 0.94 | 1.24 | 1.84 | 2.44 | 3.64 | 6.58 |
| BO | 0.75 | 0.99 | 1.29 | 1.89 | 2.49 | 3.69 | 6.63 |

**ZL1 기대 상황:**
- BP(길드Lv1): Lv1 일반 용병 3명 → 적 0.65x → 클리어 가능
- Cargo(길드Lv3): Lv3~5 고급 용병 4명 → 적 0.70x → 클리어 가능
- BO(길드Lv5): Lv5~8 희귀 용병 5명 → 적 0.75x → 클리어 가능

### 6.4 파워 구간별 설계

| 구간 | ZL | 파워 소스 | 설계 의도 |
|------|-----|----------|----------|
| **초반** | 1-10 | 레벨업 + 일반~고급 장비 | 쾌감 위주, 성장 체감 |
| **중반** | 11-25 | 레벨업 + 희귀~에픽 장비 + 강화 | 파밍 재미, 빌드 탐색 |
| **후반** | 26-40 | 레벨캡 도달 + 에픽 장비 + 강화3 | 장비 파밍 본격화 |
| **엔드** | 41-60 | 전설 장비 + 강화4~5 + 구역 특수장비 | 장비 필수 벽, 파밍 루프 |
| **극한** | 61-99 | 제작 전용 + 저주장비 트레이드오프 | 랭킹/도전, 완벽 빌드 요구 |

### 6.5 장비 파워 기여도 (강화 포함)

장비가 없으면 ZL30에서 막히도록 설계:

| 장비 세팅 | ATK 추가 | DEF 추가 | HP 추가 | 총 파워 배율 |
|-----------|---------|---------|---------|------------|
| 무장비 | 0 | 0 | 0 | 1.0x |
| 일반 3세트 | +15 | +10 | +30 | ~1.2x |
| 희귀 3세트 | +30 | +20 | +60 | ~1.5x |
| 에픽 3세트 | +45 | +30 | +90 | ~1.8x |
| 에픽+강화3 | +60 | +42 | +125 | ~2.2x |
| 전설+강화5 | +90 | +60 | +180 | ~3.0x |
| 전설+강화5+구역 | +110 | +70 | +200 | ~3.5x |
...
**→ ZL60 적 (scaleMult 4.19x):** Pitlord HP 6,285, ATK 188 → 도전적이지만 가능

### 6.6 Cargo 특수 밸런스

Cargo는 연속 웨이브 방어 → 추가 보정 필요:

| 항목 | 현재 | 수정 |
|------|------|------|
| ZL1 적 스탯 | 1.0x | **0.70x** (통일 공식) |
| Shielder HP | 300 | **200** (기본값 하향) |
| Bomber ATK | 30 | **20** (기본값 하향) |
| 초기 웨이브 간격 | 8초 | **12초** (Floor 1-3), 이후 점진 감소 |
| 열차 칸 HP | 530/3 = 176 | **300** (Floor 1 기준) |
| 웨이브 간격 최소 | 3초 | **4초** (Floor 1-10), 3초(Floor 11+) |

### 6.7 Blackout 특수 밸런스

| 항목 | 현재 | 수정 |
|------|------|------|
| ZL1 적 스탯 | 1.0x | **0.75x** (통일 공식) |
| Bone Golem HP | 400 | **250** (기본값 하향) |
| ZL1 방당 적 수 | 1-2 | **1** (depth 4까지) |
| 기습 피해 | 10% | **5%** (Floor 1-3) |

### 6.8 구현 파일 목록

1. **`balance.js`** — ZONE_SCALE 추가, MERC_MAX_LEVEL 30 확인
2. **`Mercenary.js`** — getXpToNextLevel() 변경: `50 * lv^1.5`, 레벨캡 30
3. **`enemies.js`** — getEnemyComposition scaleMult를 ZONE_SCALE 기반으로 통일
4. **`ManualBattleScene.js`** — scaleMult를 통일 공식 참조
5. **`CargoBattleScene.js`** — Floor 1 보정, 웨이브 간격 완화, 열차 HP 상향
6. **`BlackoutBattleScene.js`** — Floor 1 보정, Bone Golem 하향
7. **`cargoData.js`** — getCargoFloorScaling에 zoneBaseMult 반영
8. **`enemies.js 적 기본 스탯`** — Shielder/Bomber/BoneGolem 하향
