...
public static class AtmosphereDepthReview
{
    ...
    public static string Capture(int variant, string label)
    {
        ...
        var go=new GameObject("AtmosphereDepthReviewCamera"){hideFlags=HideFlags.HideAndDontSave};
        ...
        try{
            ...
            foreach(bool night in new[]{false,true}){
                ...
                if(variant>=4 && night){
    sun.intensity=.38f;sun.color=new Color(.66f,.58f,.85f);
    sun.transform.rotation=Quaternion.Euler(58,-35,0);
    DayNightCycle.ApplyAmbient(new Color(.20f,.17f,.27f));
    RenderSettings.fogDensity=.022f;
    foreach(var l in practicals){
        if(l.name=="PracticalLight"){l.intensity=14;l.range=8;l.color=new Color(1,.78f,.53f);}
        if(l.name=="PorchWarmLight"){l.intensity=7.5f;l.range=6.5f;l.color=new Color(1,.75f,.48f);l.shadows=LightShadows.Soft;}
    }
}
if(variant>=4 && !night){
    foreach(var l in practicals)if(l.TryGetComponent<PropLight3D>(out var phase)){l.intensity=phase.dayIntensity;l.range=l.name=="PorchWarmLight"?6.5f:8f;l.color=l.name=="PorchWarmLight"?new Color(1,.75f,.48f):new Color(1,.78f,.53f);l.shadows=l.name=="PorchWarmLight"?LightShadows.Soft:LightShadows.None;}
}
