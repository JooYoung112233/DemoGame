...
public static class InteriorFinish
{
 ...
 public static string Run(){
  ...
  // Preserve the capsule-centred root height and the current central welcome position.
  var npc=scene.GetRootGameObjects().SelectMany(g=>g.GetComponentsInChildren<InteractableObject>(true)).Single(n=>n.name=="전당포 주인");npc.transform.position=buildings.Single(b=>b.name=="Pawnshop").transform.position+new Vector3(0,.9f,-3.9f);EditorUtility.SetDirty(npc.transform);
