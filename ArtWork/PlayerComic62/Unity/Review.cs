...
public static class PlayerComicReview {
 const string Root="Assets/ChibiSurvivor/Player/Painted62";
 static string Out=>Path.GetFullPath("../ArtWork/PlayerComic62/Unity");
 public static string Run(){
 if(!Application.isPlaying||TopDownPlayer.Instance==null)throw new Exception("Player runtime required before texture import/review");
 var original=new Dictionary<Material,Texture>();
 try {
 foreach(var n in new[]{"Player","PlayerGear"}){
 var path=Root+"/Textures/"+n+"_Comic_B.png";
 File.Copy(Path.GetFullPath("../ArtWork/PlayerComic62/Textures/"+n+"_Comic_B.png"),path,true);AssetDatabase.ImportAsset(path,ImportAssetOptions.ForceSynchronousImport);
 ...
 Capture("A_Previous");
 foreach(var pair in original){string n=pair.Key.name=="Player_Painted"?"Player":"PlayerGear";pair.Key.SetTexture("_BaseMap",AssetDatabase.LoadAssetAtPath<Texture2D>(Root+"/Textures/"+n+"_Comic_B.png"));}
 Capture("B_Comic");
 ...
 if(!Application.isPlaying||TopDownPlayer.Instance==null)throw new Exception("Player runtime required before apply");
 foreach(var n in new[]{"Player","PlayerGear"}){var m=AssetDatabase.LoadAssetAtPath<Material>(Root+"/Materials/"+n+"_Painted.mat");var texture=AssetDatabase.LoadAssetAtPath<Texture2D>(Root+"/Textures/"+n+"_Comic_B.png");if(texture==null)throw new Exception("Missing Comic B texture");m.SetTexture("_BaseMap",texture);EditorUtility.SetDirty(m);AssetDatabase.SaveAssetIfDirty(m);}
 ...
 var valid=mats.Length==17&&mats.All(m=>AssetDatabase.GetAssetPath(m.GetTexture("_BaseMap")).EndsWith("_Comic_B.png"));
 ...
 return "Comic B texture references saved and applied to 17 live player slots";
 }
 ...
 var target=p.transform.position+Vector3.up*.92f;float faceYaw=Mathf.Atan2(p.FacingDirection.x,p.FacingDirection.y)*Mathf.Rad2Deg+180f;var q=Quaternion.Euler(view.Item2,view.Item3+faceYaw,0);c.transform.SetPositionAndRotation(target-q*Vector3.forward*15,q);c.nearClipPlane=13.5f;c.orthographicSize=1.23f;c.aspect=.9f;c.targetTexture=rt;c.Render();RenderTexture.active=rt;
