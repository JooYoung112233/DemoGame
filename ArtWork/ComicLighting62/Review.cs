...
public static class ComicLightingReview
{
    ...
    {
        if(!p.TryGet<ColorAdjustments>(out var c))c=p.Add<ColorAdjustments>(true);c.contrast.Override(6);c.saturation.Override(5);c.postExposure.Override(.05f); if(p.TryGet<FilmGrain>(out var grain))grain.intensity.Override(0);
        if(p.TryGet<Vignette>(out var v))v.intensity.Override(.10f);
        if(p.TryGet<Bloom>(out var b)){b.intensity.Override(.04f);b.threshold.Override(1.4f);b.scatter.Override(.35f);}
    }
    ...
    public static string Capture(int variant, string label)
    {
        ...
        var practicalStates=practicals.Select(l=>new {l.enabled,l.intensity,l.range,l.color,l.shadows}).ToArray();
        var go=new GameObject("ComicLightingReviewCamera"){hideFlags=HideFlags.HideAndDontSave};
        ...
        try{
            for(int i=0;i<volumes.Length;i++){volumes[i].profile=copies[i];if(variant>0)Grade(copies[i]);}
            foreach(bool night in new[]{false,true}){
                ...
                sun.color=night?w.nightLightColor:w.dayLightColor;
                sun.intensity=night?w.nightIntensity:w.dayIntensity;
                DayNightCycle.ApplyAmbient(night?w.nightAmbientColor:w.dayAmbientColor);
                RenderSettings.fog=night?w.nightFog:w.dayFog;
                RenderSettings.fogColor=night?w.nightFogColor:w.dayFogColor;
                RenderSettings.fogDensity=night?w.nightFogDensity:w.dayFogDensity;
                // Positive variants reproduce the candidate; zero uses saved assets.
if(variant>0){
 sun.intensity=night?.30f:.82f;sun.color=night?new Color(.63f,.70f,.88f):new Color(1,.98f,.94f);
 DayNightCycle.ApplyAmbient(night?new Color(.13f,.16f,.23f):new Color(.25f,.28f,.32f));
 if(night){RenderSettings.fogColor=new Color(.06f,.08f,.12f);RenderSettings.fogDensity=.016f;}
 foreach(var l in practicals){
  if(l.name=="PracticalLight"){l.intensity=night?9:2.5f;l.color=new Color(1,.87f,.69f);}
  if(l.name=="PorchWarmLight"){l.intensity=night?2.8f:.65f;l.range=4.5f;l.color=new Color(1,.87f,.69f);}
  if(l.name=="Bulb"){l.intensity=1.8f;l.color=new Color(1,.92f,.80f);}
 }
}
foreach(string view in new[]{"Forecourt","Player","Warden","Interior","Town"}){
                    var target=view=="Player"?TopDownPlayer.Instance.transform.position+Vector3.up*.7f:view=="Forecourt"?new Vector3(34,0,39):view=="Warden"?new Vector3(50,.5f,16):view=="Interior"?new Vector3(34,.5f,45):new Vector3(44,0,32);
                    cam.orthographicSize=view=="Player"?2.2f:view=="Town"?13:view=="Forecourt"?7:3.6f;cam.transform.position=target-cam.transform.forward*25;
            ...
            for(int i=0;i<practicals.Length;i++){var l=practicals[i];var s=practicalStates[i];l.enabled=s.enabled;l.intensity=s.intensity;l.range=s.range;l.color=s.color;l.shadows=s.shadows;}
