...
public static class WelcomePaths
{
 public static string Run(){
  ...
  foreach(var b in UnityEngine.Object.FindObjectsByType<BuildingInterior>().Where(b=>b.name=="Pawnshop")){
   ...
   spec["approachTargets"][0]=new JArray(0,-5.25);var targets=new List<object>();int i=0;foreach(var t in spec["approachTargets"]){if(i++==1)continue;int x=Mathf.Clamp(Mathf.RoundToInt(((float)t[0]+width*.5f)/step-.5f),0,nx-1);int z=Mathf.Clamp(Mathf.RoundToInt(((float)t[1]-center+depth*.5f)/step-.5f),0,nz-1);targets.Add(new{x,z,reachable=seen[x,z]});}
  ...
  var json=JsonConvert.SerializeObject(rows,Formatting.Indented);File.WriteAllText(Path.GetFullPath("../ArtWork/PawnshopWelcome62/PathValidation.json"),json);return json;
 }
}
