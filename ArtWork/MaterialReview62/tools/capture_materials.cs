...
try{
 foreach(var night in new[]{false,true}){
  ...
  foreach(var view in new[]{"Town","Warden","Pawnshop"}){
   var target=view=="Town"?new Vector3(44,0,32):view=="Warden"?new Vector3(50,.5f,16):new Vector3(34,.5f,45);
   cam.orthographicSize=view=="Town"?13:3.6f;cam.transform.position=target-cam.transform.forward*25;
   ...
   System.IO.File.WriteAllBytes("D:/Demo/ArtWork/MaterialReview62/"+label+"_"+view+(night?"_Night":"_Day")+".png",texture.EncodeToPNG());UnityEngine.Object.DestroyImmediate(texture);
  }
 }
}finally{
