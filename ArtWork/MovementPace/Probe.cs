...
public static class MovementPaceProbe
{
    static string Root => Path.GetFullPath(Path.Combine(Application.dataPath,"../../ArtWork/MovementPace"));
    ...
    static IEnumerator Measure(string file, bool allModes)
    {
        ...
        try
        {
            ...
            foreach(string mode in file=="crouch"?new[]{"crouch"}:allModes?new[]{"walk","run","crouch"}:new[]{"walk"})
            {
                if(mode=="crouch") yield return TapC();
                var inputs=new[]{Vector2.up,Vector2.right,Vector2.down,Vector2.left,new Vector2(1,1),new Vector2(-1,1),new Vector2(1,-1),new Vector2(-1,-1),new Vector2(.3f,.4f)};
