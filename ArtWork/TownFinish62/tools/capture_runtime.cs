...
try{
 foreach(var after in new[]{false}){
  ...
  foreach(var view in new[]{"Forecourt"}){
   ...
   cam.targetTexture=rt;cam.Render();RenderTexture.active=rt;var tex=new Texture2D(1280,960,TextureFormat.RGB24,false);tex.ReadPixels(new Rect(0,0,1280,960),0,0);tex.Apply();System.IO.File.WriteAllBytes(stage+"/"+"Runtime"+"_"+view+".png",tex.EncodeToPNG());UnityEngine.Object.DestroyImmediate(tex);
  }
 }
}finally{
