...
public static class BackpackReview
{
    static string Root=>Path.GetFullPath(Path.Combine(Application.dataPath,"../../ArtWork/BackpackLight62"));
    public static string Capture(string label,bool diagnostics)
    {
        ...
        var spillLocal=spill.transform.localPosition;var view=p.GetComponentsInChildren<Transform>().Single(t=>t.name=="Character3D");var viewRot=view.localRotation;
        ...
        var go=new GameObject("BackpackReviewCamera"){hideFlags=HideFlags.HideAndDontSave};
        ...
        try{
            foreach(bool night in new[]{false,true}){
                ...
                foreach(int yaw in new[]{0,180}){
                    ...
                    foreach(string test in diagnostics?new[]{"Before","NoGlow","NoSpill","Tuned","LowSpill"}:new[]{label}){
                        spill.transform.position=lamp.transform.position+(diagnostics?(test=="LowSpill"?Vector3.down*.65f:Vector3.zero):Vector3.up*new SerializedObject(lamp).FindProperty("spillHeightOffset").floatValue);glow.intensity=test=="NoGlow"?0:test=="Tuned"?.04f:gi;
                        spill.intensity=test=="LowSpill"?(night?.136f:.08f):test=="NoSpill"?0:test=="Tuned"?(night?.25f:.08f):si;
                        ...
                        try{tex.ReadPixels(new Rect(0,0,960,720),0,0);tex.Apply();File.WriteAllBytes(Path.Combine(Root,test+"_"+(night?"Night":"Day")+(yaw==0?"_Back":"_Front")+".png"),tex.EncodeToPNG());}
                        finally{UnityEngine.Object.DestroyImmediate(tex);}
                    }
                }
            }
        }finally{
            view.localRotation=viewRot;spill.transform.localPosition=spillLocal;
