...
후속 형태 수정: 최초 납작한 헬멧 시안은 사용자 피드백으로 교정했다. 현재 모델은 챙/정수리 패치 제거, 둥근 헬멧과 얼굴을 따르는 테두리, 좁아진 얼굴/턱·뒤로 기울인 이마, 슬림한 상완 소매와 몸에 붙는 파우치다. 기존 뼈/모션/아틀라스/본체 가중치는 유지한다. `BeforeRevision.jpg`는 교정 전 이력이며, 최신 비교는 `RangedRevisionComparison.jpg`다.
...
2026-09-11 재검수: 정면·측면·후면 대기와 걷기/달리기 5개 렌더를 직접 확인했다. 이마를 가르던 헬멧 밑면, 관자놀이 돌출, 목 연결 틈을 보완한 결과다. 현재 3,342삼각형이며 23본·원본 4액션·202프레임 변형/FBX 검사를 다시 통과했다. 총을 든 자세와 Unity 재질 검증은 아직 남아 있다.
