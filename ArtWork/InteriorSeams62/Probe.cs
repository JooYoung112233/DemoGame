...
{
 static string Root=>Path.GetFullPath("../ArtWork/InteriorSeams62");
 ...
 static IEnumerator Test(){
  ...
  try{
   ...
   foreach(var b in buildings.OrderBy(b=>b.name)){
    ...
    var roof=b.transform.Find("Roof_Art02").GetComponentInChildren<Renderer>();bool hidden=!roof.enabled; var signs=new[]{"전당포","수리점","의료소","가구점","암시장"}; var keys=new[]{"Pawnshop","Repair","Medical","Furniture","BlackMarket"}; var sign=UnityEngine.Object.FindObjectsByType<TextMesh>().Single(t=>t.name=="Sign_"+signs[Array.IndexOf(keys,b.name)]).GetComponent<Renderer>(); bool signHidden=!sign.enabled;
    ...
    bool exited=!b.PlayerInside;bool restored=roof.enabled; bool signRestored=sign.enabled;
    rows.Add(new{name=b.name,distance,y,entered,roofHidden=hidden,exited,roofRestored=restored,signHidden,signRestored,npcTarget=target,dialogueOpened=dialogue,pass=distance>1.7f&&Mathf.Abs(y)<.02f&&entered&&hidden&&exited&&restored&&signHidden&&signRestored&&(b.name!="Pawnshop"||dialogue)});
   }
  }finally{
